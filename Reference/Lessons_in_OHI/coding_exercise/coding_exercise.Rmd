...
You will have 24 hours to complete this activity. You will work directly in this Rmarkdown document:

* writing code to wrangle, analyze, and visualize the data in the gray "code chunk" portion of the document 
* answering questions (and providing additional insights/thoughts) in the text portion of the document

Note: If you are unfamiliar with Rmarkdown, that is ok....just add your code to the gray code chunks and run it like you would in any .R file. 

As well as answering the specific questions throughout the document, be sure to document your workflow.  This information can be included in the code chunks as well as the text of the Rmarkdown file.  

Once you have finished, please add your last name to the file name (e.g., frazier_coding_exercise.Rmd) and send to halpern@nceas.ucsb.edu. You can also provide a knitted html version of this file as well, but this is not necessary.

You will be evaluated on: coding ability, documentation of code, and communication of results.     


Please contact frazier@nceas.ucsb.edu if you have any questions about this exercise.


## Packages

Add the packages you will use to the following code chunk.  We start by loading the `readr` package, which we will use to read in dataset.
...
You will be working with a dataset describing global Ocean Health Index (http://ohi-science.org/ohi-global/) scores for the 2018 assessment for:
...
For each country and goal (i.e., the 10 goal + 1 Index score), calculate the average score across all years (note: each country should have N=11 scores when finished).
