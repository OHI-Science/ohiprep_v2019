...
[REFERENCE RMD FILE: https://cdn.rawgit.com/OHI-Science/ohiprep/master/globalprep/mar/v2017/mar_dataprep.html]?bro
...
New year of FAO mariculture yield data for 2016. Included some seaweed species for the 2018 assessment (see species_list.csv). No changes to sustainability. Check for changes in genetic escapee data or general methods. We will be updating our approach to reference points. All the reference point calculations occur in the ohi-global toolbox.
...
# mar <- filter (mar, exclude_no_seaweed==0) 
...
## 3743 of these out of 26778+3743 cases had NA values converted to 0
...
```{r}
summary(mar_sus_gf) # no NA's in Sust_gf_taxon column
...
## Remove duplicates
# mar_sus_final <- mar_sus_final %>%
#  group_by(rgn_id, species, species_code, taxa_code, year, gapfill_sus, gapfill_fao, tonnes, Sust) %>%
#  unique() %>%
#  ungroup()
...
#anyDuplicated(tonnes_esc)
## Remove duplicates
# tonnes_esc <- tonnes_esc %>%
#  group_by(rgn_id, species, species_code, year, gapfill_escapees, tonnes, Escapees) %>%
#  unique() %>%
#  ungroup()
...
compare <- full_join(old, new, by="rgn_id")
compare <- na.omit(compare)

## Compare genetic escapes pressure scores
plot(compare$prs_score_old.x, compare$prs_score_old.y)
...
plot(test$Maric_sustainability.x, test$Maric_sustainability.y)

## Compare new/old genetic escapees scores
plot(test$Genetic.escapees.x, test$Genetic.escapees.y)
...
exclude_no_sw <- read.csv("../v2018/output/GenEsc.csv") %>%
  filter(year == 2015) %>% # compare the most recent shared year between old and new
  select(rgn_id, prs_score_old=pressure_score)

exclude_no_nei <- read.csv("../v2018/output/GenEsc.csv") %>%
  filter(year == 2015) %>% # compare the most recent shared year between old and new
  select(rgn_id, prs_score_old=pressure_score)

## Compare exclusion method 1 with original exclusion method
compare <- full_join(exclude, exclude_no_sw, by="rgn_id")
compare <- na.omit(compare)
## Compare genetic escapes pressure scores
plot(compare$prs_score_old.x, compare$prs_score_old.y)

## Compare exclusion method 3 with original exclusion method
compare <- full_join(exclude_no_nei, exclude_no_sw, by="rgn_id")
compare <- na.omit(compare)
## Compare genetic escapes pressure scores
plot(compare$prs_score_old.x, compare$prs_score_old.y)

## Compare exclusion method 1 with method 3
