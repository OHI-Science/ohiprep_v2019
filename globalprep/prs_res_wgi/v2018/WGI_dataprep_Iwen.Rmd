...
Countries without 4 or more indicator are cut (gapfilled later).
...
d_gap_fill  <- d %>%
  ...
  group_by(country, year) %>% # count NA values for the num of indicators after last gap-fill (some values will be gapfilled by other years of data)
  mutate(NA_count_post_gf1 = sum(is.na(value)))     


## cut regions with < 4 indicators (if any) to calculate score:
...
d_calcs  <-  d_gap_fill %>%
  ...
  summarize(score_wgi_scale = mean(value, na.rm=T),
            ...
            NA_post_gf_1 = mean(NA_count_post_gf1)) %>% # number of NA across indicators, post-gapfill
  ungroup() 
...
## Resume gapfilling missing data (part 2)
