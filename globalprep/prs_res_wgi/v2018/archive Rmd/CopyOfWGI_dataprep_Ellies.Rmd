...
#Summary
This script downloads WGI data and prepares it for a pressures layer (1 - WGI) and a resilience data layer.  
...
# comment out when knitting: # is there a way to build this so you don't have to manually comment/uncomment when running vs knitting?
...
# WDIsearch('violence')# general search # not sure what this comment is about
key_voice = WDI(
...
# archived record of raw data: 
# write.csv(d, 'raw/worldbank_wgi_from_wdi_api.csv', row.names=FALSE) # explain this more? when does this get uncommented?
...
Countries without 4 or more indicators are cut (gapfilled later). ***why 3 or fewer? why not just keep what is there and gapfill around that?***
```{r}
...
# Gapfilling, part 2: assigning territorial region value from parent country
This gapfilling occurs when we use assign a territorial region the value of their parent country. *** in section below say that "but "in this case, we averaged the sovereign country and the available territorial values;" is this gapfilling section still up to date or is the "check the region" portion not correct?***
...
# Check the data 
Comparing this year's values against last year's.  These should be the same unless there have been updates to WGI source data or a change to methods.  For this year, there was a small change that effected a few territorial regions.  In the past, we used the sovereign country value, but in this case, we averaged the sovereign country and the available territorial values. For example, **?**
...
# Save the data
Gapfilling and score data are saved for each scenario year. *** still confused about "scenario year" vs "data year" vs "download year"; does this mean data are saved every year the scenario is completed, or data for every year the scenario is completed is saved in a given assessment year?***

```{r}

# have to comment this out to knit it; why/is this an issue?
