...
#Summary
This script downloads WGI data and prepares it for Weakness of Governance 'ss_wgi' (pressure calculated as 1-WGI) and Strength of Governance 'wgi_all' (resilience calculated as the WGI) data layesr.


#Updates from previous assessment

Incorporates one more year of data (year 2016)
...
Using the functions from the WGI package you can search within the data set
Example: `WDIsearch('violence')`


```{r}

## get description of variables in the data.frame
...
Build one data frame with the values for the 6 indicators indentified above.
...
# write.csv(d, 'raw/worldbank_wgi_from_wdi_api.csv', row.names=FALSE)
...
d_gap_fill  <- d %>%
  ...
  mutate(NA_count_c_y = sum(is.na(value))) %>%  # get count of NA values per countries per year for the indicators prior to gapfilling, max value is 6
  ungroup() %>%
  group_by(country, indicator) %>%  
  # this section gap-fills with the mean of values across years within the same region/indicator
  mutate(ind_mean_c_i = mean(value, na.rm=TRUE)) %>% #calculates the mean per indicator per contry
  ungroup() %>%
  mutate(value = ifelse(is.na(value), ind_mean_c_i, value)) %>% #gapfills value column with ind_mean_c_i
  group_by(country, year) %>%
...
countries_no_data <- d_gap_fill %>%
  filter(NA_count_post_gf1 > 3) #in this data.frame there are no countries with less than 4 indicators
...
wgi_range = c(-2.5, 2.5) #range in value from the 6 dimensions of governance of the WGI
...
#check which country_year were not gapfilled
...
#Error: `by` can't contain join column `year` which is missing from RHS
...
old2015 <- read.csv('../v2016/output/wgi_res_2016.csv') %>%
