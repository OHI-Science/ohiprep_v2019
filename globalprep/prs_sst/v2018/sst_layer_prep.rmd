...
```{r compare rescaled diffs between assessments}
...
```{r load data for zonal extraction}
...
# zones <- raster(file.path(rast_loc, "sp_mol_raster_1km.tif")) # as of v2018 use 'zones' (regions_eez_with_fao_ant.tif) sourced from spatial.common
```
...
```{r gif of rescaled ice masked sst}
...
setdiff(regions_stats$zone, rgn_data$sp_id) # check
setdiff(rgn_data$sp_id, regions_stats$zone) # check
...
data <- read.csv(file.path(dir_output, "rgn_sst_prs.csv"))
```


## Write final pressure layer and gapfilling record

```{r sst pressure layer}
...
```{r gapfilling record}
## note sst has no gapfilling...
sst <- read.csv("output/sst_eez_2018.csv")
sst <- mutate(sst, pressure_score = 0)
write.csv(sst, "output/sst_eez_2018_gf.csv", row.names = FALSE)
```
