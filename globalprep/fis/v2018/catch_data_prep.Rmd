...
new_filt <- new %>% 
  ...
  rename(new_log_catch = log_mean_catch) %>% 
  filter(Year == 2014) %>% 
...
old_filt <- old %>% 
  ...
  rename(Year = year) %>% 
  mutate(log_mean_catch = log(mean_catch+1)) %>% 
  rename(old_log_catch = log_mean_catch) %>% 
  filter(Year == 2014) %>% 
...
## Plot with plotly to see region id when hovering over points
plot_check <- ggplot(check, aes(old_log_catch, new_log_catch, col = rgn_id)) +
  geom_point(alpha = 0.4) +
  geom_abline(col="red") +
  ggtitle("Catch Comparison for 2014")
ggplotly(plot_check)
