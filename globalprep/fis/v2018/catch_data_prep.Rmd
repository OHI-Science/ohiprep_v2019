...
output_df <- catch %>% 
    ...
    dplyr::mutate(catch_prop = CatchTotal * area) %>% # NAs produced here due to not having a Cell ID that corresponds to a cell id in cells data
...
new_taxa <- filter(stock_rgn, paste(stock_rgn$TaxonName, stock_rgn$CommonName) %in% no_taxonkey) %>%
...
setdiff(paste(stock_rgn$TaxonName, stock_rgn$CommonName), # any here need to be corrected
  paste(taxonkey$TaxonName, taxonkey$CommonName))
