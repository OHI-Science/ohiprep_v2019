...
Need taxon key to easily remove higher level (e.g. genus) taxonomic catch data. Unique taxon key was extracted from Watson 2017 catch data in [`catch_taxon_key.Rmd`](https://mazu.nceas.ucsb.edu/rstudio/files/github/ohiprep_v2018/globalprep/fis/v2018/catch_taxon_key.html). Add them to the stock catch by region.

Combine taxa with all catch regions data. Read in taxon key and stock catch (tons) by region data.
```{r, eval=FALSE}
...
## create dataset ready to run through catch-only models
    stks <- df %>%
              ...
              dplyr::select(year = Year, TaxonName, CommonName, fao_rgn, stock_id, Taxonkey, Resilience, tons)
...
# write.csv(stks, file = 'data/stock_catch_landings.csv', row.names = FALSE)
...
#stks_landings = read.csv('data/stock_catch_landings.csv')
...
# write.csv(mean_catch_toolbox, "data/mean_catch_landings.csv", row.names=FALSE)
...
```{r, eval=FALSE}
new <- read.csv("data/mean_catch_landings.csv")
new_filt <- new %>% 
  mutate(new_log_catch = log(mean_catch+1)) %>% 
  filter(year == 2014) %>% 
  dplyr::select(rgn_id, stock_id_taxonkey, new_log_catch) 
