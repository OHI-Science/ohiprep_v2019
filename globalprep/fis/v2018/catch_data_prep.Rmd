...
Using Watson 2018 data this year. Catch data now goes from 1950 - 2015. Previously catch data was only available through 2014 from the Sea Around Us (SAUP). No longer using SAUP to create cells.csv - took a raster template of Watson (watson_cell_id_rast.tif) created by Casey for a different project.
...
#old_path = file.path(dir_M, "git-annex/globalprep/_raw_data/SAUP/d2017/annual_data")
...
catch <- readRDS(paste0(fis_path, "annual_catch/","Catch_2015.rds"))
dim(catch)
summary(catch)
...
Since we are using a new data source, we recreate the cells.csv file, which will include cell ids and corresponding OHI and FAO region ids, which is later used to align catch data with appropriate regions. This file is prepared in clean_cells.Rmd.
...
#   dplyr::select(-X1)
# 
# dim(cells)[1]-dim(old_cells)[1] # there are 520 more rows maybe due ot extra data
...
(Iwen aug 23 CHANGES: removed taxon_key, changed catch_sum to Landings and ignored discards right?)
```{r, eval=F}
...
output_df <- readRDS(df) %>%
        ...
        dplyr::mutate(catch_prop = Landings * area) %>%
        group_by(Year, rgn_id, fao_id, TaxonName, CommonName) %>%
...
all_data <- map_df(data_files, stock_rgn_catch)

## this is a large file (150 MB) so it is saved on the server
write.csv(all_data, file =  file.path(dir_M,'git-annex/globalprep/fis/v2018/int/stock_catch_by_rgn.csv'), row.names=FALSE)
...
## create dataset ready to run through catch-only models
    stks <- df %>%
              ...
              dplyr::group_by(stock_id,year,fao_rgn,taxon_scientific_name,taxon_common_name,taxon_key) %>%
              ...
              dplyr::filter(avg_ann_catch >= min_tons,        #keep only those stocks that meet our conditions
                              nyrs >= min_yrs) %>%
              dplyr::left_join(taxon_res, by = "taxon_common_name") %>%                  #add resilience information
...
no_res <- filter(stks, is.na(Resilience)) %>%
          dplyr::select(taxon_scientific_name, taxon_common_name) %>%
          distinct()
...
catch <- read_csv(file.path(dir_M,'git-annex/globalprep/fis/v2018/int/stock_catch_by_rgn.csv')) %>%
...
Check out the data
```{r, eval=FALSE}
data.frame(filter(mean_catch, stock_id == "Elasmobranchii-57" & rgn_id==1))
...
Compare with last year - should be the same
```{r}
