...
RAM <- RAM_filt %>% 
  ...
  mutate(fao_id = 
           case_when(
             ...
             str_detect(stocklong, paste(c("China rockfish Northern", "China rockfish Central", "Blue shark North Pacific"), collapse = "|")) ~ 67,
             str_detect(stocklong, paste(c("China rockfish Southern", "Striped marlin North Pacific"), collapse = "|")) ~ 77,
             ...
             str_detect(stocklong, "Cobia Gulf of Mexico") ~ 31
           )) %>%
  ...
  mutate(fao_id_2 =
           case_when(
             ...
             str_detect(stocklong, paste(c("China rockfish Central","Blue shark North Pacific"), collapse = "|")) ~ 77,
             str_detect(stocklong, "Striped marlin North Pacific") ~ 87,
             str_detect(stocklong, paste(c("Blueline tilefish", "Cobia Southern"), collapse = "|")) ~ 31
             )) %>% 
  
  mutate(fao_id_3 =
           case_when(
             str_detect(stocklong, "Blue shark North Pacific") ~ 61
           )) %>% 
  
  mutate(fao_id_4 =
           case_when(
             str_detect(stocklong, "Blue shark North Pacific") ~ 71
           )) %>% 
  ...
  mutate(rgn_id = 
           case_when(
             ...
             str_detect(stocklong, paste(c("^Haddock Georges", "^China rockfish.*Pacific Coast$", "Blue shark North Pacific", "^Monkfish", "Yellowtail flounder","Atlantic Coast","Atlantic coast"), collapse = "|")) ~ 163,
