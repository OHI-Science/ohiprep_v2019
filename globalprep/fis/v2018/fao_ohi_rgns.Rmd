...
#   dplyr::select(stockid, year, ram_bmsy = BdivBmsypref, assessid)
#   dplyr::filter(!is.na(BdivBmsypref)) %>%
#   dplyr::filter(year > 1979)

## check number of unique entries
new_stockid <- unique(ram_bmsy$stockid) # 431 unique entries
...
newStocks <- setdiff(new_stockid, old_stockid) # there are 182 diff stock names

## Compare diff when using `timeseries` table with diff when using `timeseries_values_views` for RAMv4.40 
# newStocks2 <- setdiff(new_stockid, old_stockid)
# setdiff(newStocks2, newStocks)
