---
title: "Identify FAO and OHI regions for RAM stocks"
author: "Iwen Su"
...
Assign FAO and OHI region ids for each stock (`stockid`) listed. Some stocks have multiple assessments, assign them the same region ids and retain that column information (`assessid`).
...
ram_bmsy_old <- data.frame(timeseries) %>%
  ...
  dplyr::mutate(assess_num = length(unique(assessid))) # should all have a value of 1

## check unique values - should be 1-to-1-to-1
old_stockid <- unique(ram_bmsy_old$stockid) # 257 unique entries
# old_stocklong <- unique(ram_bmsy_old$stocklong) # 257 unique entries
# old_assessid <- unique(ram_bmsy_old$assessid) # 257 unique entries
...
ram_bmsy <- data.frame(timeseries) %>%
  ...
  dplyr::select(assessid, year=tsyear, ram_bmsy = tsvalue, stockid, stocklong) %>% 
  dplyr::group_by(stockid) %>% 
  dplyr::mutate(assess_num = length(unique(assessid))) # checked that the number matched up with stocks I checked in data_check.Rmd

## check number of unique entries, confirms that for some stocks there are multiple assessments this year
new_stockid <- unique(ram_bmsy$stockid) # 353 unique entries
# new_stocklong <- unique(ram_bmsy$stocklong) # 353 unique entries
# new_assessid <- unique(ram_bmsy$assessid) # 473 unique entries

```

Check differences

Make sure to check for stock name changes from old to new. Check documentation in mazu (e.g. Stock Change History). A few outdated stock names. Can remove unused `stockid`s in RAM_data_prep.Rmd if needed. 
```{r}

## find newly added stocks
newStocks <- setdiff(new_stockid, old_stockid) # there are 101 diff stock names
...
## some stockid values are repeated in more than one observation due to multiple assessid years (see assess_num column)
id_new <- ram_bmsy %>% 
  select(assessid, stockid, stocklong, assess_num) %>% 
  distinct()
...
RAM_rgns_new <- RAM_rgns_old %>% 
  full_join(id_new, by = c("assessid","stockid","stocklong","assess_num")) %>% 
  group_by(stockid) %>% 
  mutate(test_0 = length(unique(assessid)) - assess_num) # make sure there are no diffs

## check that the number of assess id hasn't changed after joining
library(validate)
check_assessid <- check_that(RAM_rgns_new, test_0 == 0)
summary(check_assessid) # should be no fails
...
Combine newly added stocks wtih ohi and fao region information to the old stock data table.
```{r}

RAM_fao_ohi_rgn <- RAM_rgns_old %>% 
  full_join(RAM_rgns_new_final, by = c("rgn_id", "fao_id", "assessid", "stockid", "stocklong", "assess_num")) %>% 
  select(-RAM_area_m2)
