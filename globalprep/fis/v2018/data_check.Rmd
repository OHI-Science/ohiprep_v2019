...
compare <- ssb_tb_check %>% 
  ...
  filter(!is.na(BdivBmsytouse) | !is.na(TBdivTBmsy) | !is.na(SSBdivSSBmsy)) # remove rows where all three are NAs

## To check that filter for all three NAs, check ARFLOUNDGA 1958, which has NAs for all three, and COD2J3KL 1850-1961; looks good

## Check number of unique stocks - should have same number after removing assessments 
...
compare <- bbmsy_touse %>% # join old timeseries with new timeseries table
  full_join(tb_ssb, by = c("stockid","stocklong","year"))
check_assess_NA <- compare %>% # after remove assessid = NA rows, should have 33,723 NAs in each 3 Bmsy categories
  filter(is.na(assessid))
