...
ram_bmsy <- data.frame(timeseries) %>%
  ...
  dplyr::select(assessid, stockid, year=tsyear, ram_bmsy = tsvalue) %>% 
  dplyr::group_by(stockid) %>% 
  dplyr::mutate(assess_num = length(unique(assessid))) %>% # check number of assessments per stock
  dplyr::ungroup() %>% 
  dplyr::group_by(assessid, stockid) %>% 
  dplyr::mutate(assess_yr = length(year)) %>% # number of years of data per assessment id
  dplyr::ungroup()
...
tmp <- ram_bmsy_gf %>%
  ...
  unique()
length(tmp$gapfilled)   # 389 stocks with at least 5 years of data in past 11 years
table(tmp$gapfilled)    
sum(table(tmp$gapfilled))  # 305 stocks have at least one year of B/Bmsy values gapfilled
...
plot <- ggplot(data = filter(ram_bmsy_gf_check2, year==2015), aes(y=ram_bmsy_predict_error, x=ram_bmsy, col = assessid)) +
  geom_point() +
  theme_bw() +
  geom_abline(slope=1, intercept=0, color="red")
...
ram_sp <- ram_bmsy_gf %>%
  left_join(data.frame(metadata), by = "assessid") %>%
  dplyr::select(scientificname) %>%
  unique() %>%
  arrange(scientificname)

# Watson species (read in the datatable that includes TaxonKey)
...
length(unique(ram_meta$assessid)) # 1252
length(unique(ram_meta$stockid)) # 1252
length(unique(ram_meta$scientificname)) # 340

length(unique(ram_spatial$assessid)) # 372
length(unique(ram_spatial$stockid)) # 358
length(unique(ram_spatial$stocklong)) # 358


setdiff(ram_spatial$assessid, ram_meta$assessid) # make sure all the spatial data has corresponding metadata (should be 0); 134 diff
diff <- setdiff(ram_spatial$stockid, ram_meta$stockid) # 9 diff 
...
Complete investigation in data_check.Rmd
