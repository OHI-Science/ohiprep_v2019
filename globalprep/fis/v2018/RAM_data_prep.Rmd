...
ram_data <- ram_spatial %>% 
  ...
  dplyr::mutate(stockid_full = paste(species, fao_id, sep="-")) %>%
  dplyr::mutate(stockid_full = gsub(" ", "_", stockid_full)) %>%
  dplyr::filter(!is.na(year)) %>%   ## these are the ones that didn't have enough ram data to make an accurate guess
  ungroup() %>%
  dplyr::select(rgn_id, stockid_full, stockid, stocklong, year, ram_bmsy, gapfilled, method) %>%
  unique()
