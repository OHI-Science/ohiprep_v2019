...
mean_catch <- read.csv("data/mean_catch.csv", stringsAsFactors = FALSE) %>% # final output from Watson catch
...
intersect(ram$stock_id, mean_catch$stock_id) #286 stocks with RAM-B/Bmsy data (although RAM is matched by fao and rgn ids)
...
data <- mean_catch %>%
  left_join(ram, by=c('rgn_id', 'stock_id', "year")) # 30105 catch records (catch from specific fao and ohi regions)

sum(!is.na(data$ram_bmsy))/nrow(data) # about 5% of catch records have RAM data
...
sum(duplicated(paste(data$rgn_id, data$stock_id, data$year, sep="_"))) # 3948 regions with multiple RAM stocks (stockid_ram) for the same species (see scientific name in stockid)
...
## check that averaging went ok - compare with values earlier (969 and 104)
...
write.csv(data_gf, "data/fis_bbmsy_gf.csv", row.names=FALSE) 
