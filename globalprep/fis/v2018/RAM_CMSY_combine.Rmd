---
title: "OHI 2018: Food Provision, Combining RAM and CMSY B/Bmsy data"
...
Have not completed this prep script yet for 2018.
...
intersect(ram$stock_id, mean_catch$stock_id) #291 stocks with RAM-B/Bmsy data (although RAM is matched by fao and rgn ids)

## Watson v CMSY
setdiff(cmsy$stock_id, mean_catch$stock_id)
setdiff(mean_catch$stock_id, cmsy$stock_id)
intersect(mean_catch$stock_id, cmsy$stock_id) #806 stocks with CMSY-B/Bmsy data
```
...
data <- mean_catch %>%
  left_join(ram, by=c('rgn_id', 'stock_id', "year")) # 561,834 catch records (catch from specific fao and ohi regions) when joined with ram increases to 564,318 because there are multiple stocks for some species 

sum(!is.na(data$ram_bmsy))/nrow(data) # about 5.5% of catch records have RAM data
sum(data$mean_catch[!is.na(data$ram_bmsy)])/sum(data$mean_catch) # about 29% of tons of catch have RAM data
...
sum(duplicated(paste(data$rgn_id, data$stock_id, data$year, sep="_"))) # 4098 regions with multiple RAM stocks (stockid_ram) for the same species (see scientific name in stockid)
...
filter(data, rgn_id == 9, year == 2001, stock_id == "Thunnus_alalunga-71") # stocks ALBANPAC and ALBASPAC are the same species; neab catch 969 tonnes
filter(data, rgn_id == 9, year == 2001, stock_id == "Tetrapturus_audax-71") # stocks STMARLINWCNPAC and STMARLINSWPO are the same species; mean catch 104 tonnes
...
## check that averaging went ok - compare with mean catch values earlier (969 and 104)
