...
intersect(ram$stock_id, mean_catch$stock_id) #288 stocks with RAM-B/Bmsy data (although RAM is matched by fao and rgn ids)
...
data <- mean_catch %>%
  left_join(ram, by=c('rgn_id', 'stock_id', "year")) # 564,198 catch records (catch from specific fao and ohi regions)
...
sum(duplicated(paste(data$rgn_id, data$stock_id, data$year, sep="_"))) # 3978 regions with multiple RAM stocks (stockid_ram) for the same species (see scientific name in stockid)
