...
sum(duplicated(paste(data$rgn_id, data$stock_id, data$year, sep="_"))) # 3225 regions with multiple RAM stocks (stockid_ram) for the same species (see scientific name in stockid)
...
filter(data, rgn_id == 9, year == 2001, stock_id == "Thunnus_alalunga-71") # stocks ALBANPAC and ALBASPAC are the same species 
filter(data, rgn_id == 9, year == 2001, stock_id == "Tetrapturus_audax-71")
```

Regions with multiple stocks of the same species will have B/Bmsy values averaged, weighted by the area of the RAM stock within the region (Iwen 9/17 - can't weight by area since we don't have it for most recent stock additions).
```{r}

data <- data %>%
  group_by(rgn_id, taxon_key, stock_id, year, mean_catch) %>%   
  summarize(ram_bmsy = mean(ram_bmsy, na.rm=TRUE),
            #weighted.mean(ram_bmsy, RAM_area_m2, na.rm=TRUE),
