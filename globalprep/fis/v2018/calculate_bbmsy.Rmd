---
title: "OHI 2018: Food Provision, Calculating fisheries stock status"
...
To calculate the status of global fish stocks from the Watson 2018 catch data we use catch-MSY (CMSY) developed by [Martell and Froese (2012)](http://onlinelibrary.wiley.com/doi/10.1111/j.1467-2979.2012.00485.x/full). This model requires catch and resilience information for each stock. Stocks are defined by FAO areas and are limited to only those records where catch is reported at the species level.
...
Read in catch data aggregated from half degree cell to OHI region [catch_data_prep.Rmd](https://cdn.rawgit.com/OHI-Science/ohiprep_v2018/master/globalprep/fis/v2018/catch_data_prep.html).
...
if (!file.exists(file.path(dir_M,"git-annex/globalprep/fis/v2018/int/cmsy-fits.rds"))) {
...
write.csv(cmsy_bbmsy,file='data/cmsy_bbmsy.csv', row.names=FALSE)

```

Explore why there are some NAs (I think non convergance)

(Iwen 8/29 - Estimates not converging at a value? review paper referenced)
```{r nas, eval=F}
