...
#   dplyr::select(stockid, stocklong, year, ram_bmsy) #10697 observations with ram data... 
...
*Note: This is only necessary for the 2019 assessment, since this is the first year we are implementing management biomass values*
...
# ram_gf_check_new <- ram_bmgt_new_final %>%
#   filter(year >= 2001) %>%
#   spread(year, ram_bmsy) 
...
# ram_bmsy_gf_new <- ram_bmgt_new_final %>%
#   filter(year >= 2001 & year <= 2015) %>%   # 2015 corresponds to the final year of Watson catch data
#   group_by(stockid) %>%
#   mutate(years_data_2005_now = length(ram_bmsy[year >= 2005])) %>%
#   mutate(years_data_2001_now = length(ram_bmsy[year >= 2001])) %>%
...
# ram_bmsy_gf_new <- ram_bmsy_gf_new %>%
#   spread(year, ram_bmsy) %>% 
#   gather("year", "ram_bmsy", -stockid, -years_data_2005_now, -years_data_2001_now) %>%
#   mutate(year = as.numeric(year)) 
...
# tmp_new <- ram_bmsy_gf_new %>%
#   dplyr::select(stockid, gapfilled) %>%
#   unique()

## check out gapfilling stats
length(tmp$gapfilled)  # 298 stocks with at least 5 years of data in past 11 years - v2019
# length(tmp_new$gapfilled) # 397 stocks with at least 5 years of data in past 11 years - v2019

sum(table(tmp$gapfilled))  # 209 stocks have at least one year of B/Bmsy values gapfilled - v2019
# sum(table(tmp_new$gapfilled))  # 222 stocks have at least one year of B/Bmsy values gapfilled - v2019
...
summary(ram_bmsy_gf)
sum(ram_bmsy_gf$ram_bmsy_predict < 0 )  # a few of the predicted B/Bmsy values go below zero.  
...
#   ungroup()
# 
# summary(ram_bmsy_gf_new)
# sum(ram_bmsy_gf_new$ram_bmsy_predict < 0 )  # a few of the predicted B/Bmsy values go below zero.  
# min(ram_bmsy_gf_new$ram_bmsy, na.rm = TRUE) #0.00263
...
# ram_bmsy_gf_final_new <- ram_bmsy_gf_new %>%
#   mutate(method = ifelse(is.na(ram_bmsy), paste0("lm, ", gapfilled), NA)) %>%
#   mutate(gapfilled = ifelse(is.na(ram_bmsy), "1", "0")) %>%
#   mutate(ram_bmsy = ifelse(is.na(ram_bmsy), ram_bmsy_predict, ram_bmsy)) %>%
#   dplyr::select(stockid, year, ram_bmsy, gapfilled, method) 
...
# write.csv(ram_bmsy_gf_final_new, "int_mgt/ram_stock_bmsy_bmgt_gf.csv", row.names=FALSE)
```
...
# plot(ram_bmsy_gf_new$ram_bmsy, ram_bmsy_gf_new$ram_bmsy_predict)
# abline(0,1, col="red")
# 
# plot(log(ram_bmsy_gf_new$ram_bmsy), log(ram_bmsy_gf_new$ram_bmsy_predict))
# abline(0,1, col="red")
# 
# mod_new <- lm(ram_bmsy ~ ram_bmsy_predict, data=ram_bmsy_gf_new)
# summary(mod_new)
```
...
# ram_sp_new <- ram_bmsy_gf_final_new %>%
#   left_join(data.frame(metadata), by = "stockid") %>%
#   dplyr::select(scientificname) %>%
#   unique() %>%
#   arrange(scientificname)
...
# there is only one new species that we need to hand check from last year (2018) to this year (2019) - Sillago flindersi


# compare names - what's in RAM that's not in Watson
# tmp_new <- sort(setdiff(ram_sp_new$scientificname, wat_sp$wat_scientificname))
# tmp2_new <- sort(setdiff(wat_sp$wat_scientificname, ram_sp_new$scientificname))
# 
# write.csv(tmp_new, "int_mgt/unmatched_RAM_species.csv", row.names=FALSE)
# write.csv(tmp2_new, "int_mgt/Watson_species_no_RAM.csv", row.names=FALSE)

# there are only two new species that we need to hand check from last year (2018) to this year (2019) - Sillago flindersi, Ammodytes spp
...
# ram_name_corr_new <- read.csv("int_mgt/RAM_species_to_Watson.csv", stringsAsFactors = FALSE) %>%
#    filter(!is.na(Watson_species))  # Watson to RAM name conversion

# ram_name_corr_new # matched species, only 7
```
...
length(unique(ram_spatial$stockid)) # 346 RAM stocks with B/Bmsy data - v2019


# ram_spatial_new <- ram_spatial %>%
#   left_join(ram_name_corr_new, by="RAM_species") %>%
#   dplyr::mutate(species = ifelse(!is.na(Watson_species), Watson_species, RAM_species)) %>%
#   dplyr::select(rgn_id, fao_id, stockid, stocklong, species, RAM_area_m2)

# length(unique(ram_spatial_new$stockid)) # 346 RAM stocks with B/Bmsy_Bmgt data - v2019
...
# ram_bmsy_gf_final_new <- read.csv("int_mgt/ram_stock_bmsy_bmgt_gf.csv")
...
setdiff(ram_spatial$stockid, ram_bmsy_gf$stockid) # these are stocks that were dropped due to insufficient years of data
 ...
 # setdiff(ram_bmsy_gf_final_new$stockid, ram_spatial_new$stockid) # should be 0: every ram stock should have ohi/fao rgn; it is NOT 0, there are 96 new stocks without ohi/fao rgns...
# setdiff(ram_spatial_new$stockid, ram_bmsy_gf_final_new$stockid) # these are stocks that were dropped due to insufficient years of data
...
# ram_data_new <- ram_bmsy_gf_final_new %>% 
#   left_join(ram_spatial_new, by="stockid") %>%
#   rename(stockid_ram = stockid) %>% 
#   dplyr::mutate(stockid = paste(species, fao_id, sep="-")) %>%
#   dplyr::mutate(stockid = gsub(" ", "_", stockid)) %>%
#   dplyr::select(rgn_id, stockid, stockid_ram, stocklong, year, RAM_area_m2, ram_bmsy, gapfilled, method) %>%
#   unique()
# 
# write.csv(ram_data_new, "int_mgt/ram_bmsy.csv", row.names=FALSE)
```
