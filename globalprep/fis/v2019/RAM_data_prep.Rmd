...
This year we have additional stocks without spatial information from Christopher Free (2017). We manually assigned ohi and fao region id information to the additional stocks in `fao_ohi_rgns.Rmd` using best available information on stock distribution and saved the file in `int/RAM_fao_ohi_rgns.csv`. Watson species names should be similar if not the same as the SAUP species names used prior the 2018 assessment. 
...
ram_bmsy <- timeseries_values_views %>%
  ...
  dplyr::select(stockid, year, ram_bmsy) #10697 observations with ram data... 

```

ram_bmsy check with after adding management
```{r}
...
ram_bmgt_check <- timeseries_values_views %>%
  select(stockid, year, TBdivTBmsy, SSBdivSSBmsy, TBdivTBmgt, SSBdivSSBmgt) %>%
  mutate(ram_bmsy = 
...
sum(is.na(ram_bmgt_check$ram_bmsy)) #21195 NAs with new methods in ram_bmsy column... 21195/35177 = ~60% of ram data is NA with new methods.. better than before. Gained 3285 rows of data.

ram_bmgt_check_final <- ram_bmgt_check %>%
  filter(!is.na(ram_bmsy)) %>%
  dplyr::select(stockid,year,ram_bmsy)

sort(setdiff(ram_bmgt_check_final$stockid, ram_bmsy$stockid)) ## there are 96 stocks added. Are these present in both the RAM and Watson data? 
```


For future reference: Consider using management B/Bmsy values in next year's assessment.

Note: In the past, we have used the B/Bmsy to use, dimensionless (BdivBmsytouse-dimensionless) values in the `timeseries_values_views` data table. According to `tsmetrics` table, this consists of TB/TBmsy (TBdivTBmsy) otherwise if NA, SSB/SSBmsy (SSBdivSSBmsy). B/Bmsy preferred values (BdivBmsypref) uses B/Bmsy (BdivBmsypref) preferably, but if NA, then uses B/Bmgt (BdivBmgtpref) values as a substitute. The below table filters for the stocks that use BdivBmgtpref values, not currently incorporated in OHI Global from RAM. Any missing fisheries stocks are currently gapfilled instead. Investigate use of these B/Bmgt values next year. Many of them are around Iceland or European stocks.
```{r, eval = FALSE}
...
species_list <- test %>% 
  ...
  unique() #105 unique stocks that don't currently get used in the OHI GLobal. Consider using next year
...
length(tmp$gapfilled)   # 296 stocks with at least 5 years of data in past 11 years - v2018
                        # 298 stocks with at least 5 years of data in past 11 years - v2019
table(tmp$gapfilled)    
sum(table(tmp$gapfilled))  # 216 stocks have at least one year of B/Bmsy values gapfilled - v2018
                           # 209 stocks have at least one year of B/Bmsy values gapfilled - v2019
...
length(unique(ram_spatial$stockid)) # 345 RAM stocks with B/Bmsy data - v2018
                                    # 346 RAM stocks with B/Bmsy data - v2019
