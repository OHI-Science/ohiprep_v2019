...
sum(is.na(ram_bmsy_check$ram_bmsy)) #24480


ram_bmgt_check <- timeseries_values_views %>%
  select(stockid, year, TBdivTBmsy, SSBdivSSBmsy, TBdivTBmgt, SSBdivSSBmgt) %>%
  mutate(ram_bmsy = 
           ifelse(!is.na(TBdivTBmsy), TBdivTBmsy, TBdivTBmgt)) %>%
  mutate(ram_bmsy =
           ifelse(is.na(TBdivTBmsy) & is.na(TBdivTBmgt), SSBdivSSBmsy, ram_bmsy)) %>%
  mutate(ram_bmsy = 
           ifelse(is.na(TBdivTBmsy) & is.na(TBdivTBmgt) & is.na(SSBdivSSBmsy), SSBdivSSBmgt, ram_bmsy)) %>%
  dplyr::filter(year > 1979)

sum(is.na(ram_bmgt_check$ram_bmsy)) #21195
```
