...
The area of blast fishing data layer is created from sapatialized bycatch by gear data provided by Watson (2017), 
!!!!!!!!! YS OR NO? and net primary production data from the Vertically Generalized Production Model
...
foreach(yr = years) %dopar%{ #yr = 2014
 ...
 blast <- raw %>%
          ...
          rowwise() %>%
          dplyr::mutate(catch = sum(LSF_CR, SSF_CR, IUU_CR, Discards_CR, na.rm=TRUE) * 1000) %>% #there shouldnt be NAs but just in case, converting from tonnes to kg to get rid of low values
          dplyr::group_by(Seq) %>%
          dplyr::summarise(cell_catch = sum(catch))
          
    #rasterize catch by swapping cell ids with (calls the suap raster we made above)
raster::subs(saup_rast, blast, by = 'Seq', which = 'cell_catch', subsWithNA=TRUE, filename = paste0('int/blast/annual_catch/blast_',yr,'.tif'), overwrite=TRUE) 
