...
#### 2018 Assesment
Data source updated the way they report the data. This year warnings are reported for every country. We input the data manualy to an excel file, converted into csv and wrangle the data to merge with data for previous assesments.
...
Step 2: Manually chack for every country if there is any regional warning and the risk factor in each case. If different subregions within a country have different warnings, these are put on two different lines.
Eg:
...
warn_raw <- read.csv('raw/tr_travelwarning_2018_raw.csv') %>% 
  ...
  mutate(level = str_extract(level, '[1,2,3,4]')) %>% 
  filter(level!= 1)
...
scores = data.frame(category = c("inform", "risk", "avoid_nonessential", "avoid_all", "gtfo"), score = c(0, 0.25, 0.75, 1, 1)) 
...
warn <- read.csv('raw/tr_travelwarnings_2017.csv') %>%
  ...
  mutate(warning_count = n()) # column 'regions' is number of warnings issued that year for each country

warn2 <- warn %>%
  mutate(score = ifelse(warning_count %in% 1 & regional %in% 1, score*0.5, score)) %>% # adjust country-year observations that only have had a single warning that was only applied to specific regions
