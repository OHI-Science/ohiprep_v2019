...
warn_improved <- warn_raw %>% 
  ...
  dplyr::mutate(rgn_name = ifelse(rgn_name == "Israel" & !is.na(regional), "disputed area", rgn_name))
...
#Identify which regions are not part of the U.S. State Department warning data for 2018
...
tw_2018_complete <- georegions %>%
  dplyr::left_join(tw_2018) %>%
  dplyr::left_join(georegion_labels, by = 'rgn_id') %>%
  dplyr::select(rgn_id, year, multiplier)

tw_2018_NA <- tw_2018_complete %>% 
