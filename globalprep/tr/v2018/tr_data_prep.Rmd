...
#setwd('globalprep/tr/v2018') #comment out when knitting
...
warn <- read.csv('raw/tr_travelwarnings_2017.csv') %>%
  ...
  mutate(regions = n()) # column 'regions' is number of warnings issued that year for each country

warn2 <- warn %>%
  mutate(score = ifelse(regions %in% 1 & regional %in% 1, score*0.5, score)) %>% # adjust country-year observations that only have had a single warning that was only applied to specific regions
...
expand.grid.df <- function(...) Reduce(function(...) merge(..., by=NULL), list(...)) # first merge df then reduce
