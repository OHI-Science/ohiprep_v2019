...
Step 1: Copy and paste the data for each country: https://travel.state.gov/content/passports/en/alertswarnings.html
...
warn_complete <- warn_improved %>% 
  bind_rows(warn_old_no_subs) %>%
  bind_rows(warn_old_add_rgns) 
...
--------------- | ------------ | -------------------
Level 1 | 0 (no penalty) | Exercise Normal Precautions: This is the lowest advisory level for safety and security risk. There is some risk in any international travel. 
Level 2 | 0.25  | Exercise Increased Caution:  Be aware of heightened risks to safety and security. 
Level 3 | 0.50  | Reconsider Travel: Avoid travel due to serious risks to safety and security.
Level 4 | 1 (full penalty) | Do Not Travel:  This is the highest advisory level due to greater likelihood of life-threatening risks. 

```{r}
scores <-  data.frame(level = c(1, 2, 3, 4), multiplier = c(1, 0.75, 0.25, 0)) 

warn_complete = warn_complete %>%  
...
filter(warn_complete, warning_count>1)
...
write.csv(warn_complete, "intermediate/warning.csv", row.names=FALSE)


```


Step 4: Convert names to OHI regions and clean. 
...
Step 5: Identify inhabited regions with no U.S. travel warning data and see if this information is available from the Canadian Government website and add warnings to the raw data csv. Make sure to label the column indicting the data source. 
