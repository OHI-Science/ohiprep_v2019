...
* Travel warnings: (U.S. State Department or Canada's Government Travel Advise and Advisory)
...
Travel Warning data form the U.S State department has updated the way data is reported. This year all countries have a travel warning that ranges from level 1 (normal precautions) to level 4 (do not travel). Within each general warning for a country there are regional warnings which are included in the raw data. For more details see the travel_warning section of this data_prep. 
...
* Proportion of jobs in tourism - WTTC data reported until 2018 (downloaded: 07/12/2018)
...
warn_improved <- warn_improved %>% 
  ...
  dplyr::mutate(rgn_name = ifelse(stringr::str_detect(country,"Line Islands"), "Line Group", rgn_name)) %>% 
...
##Note: Discounting scores for regions that only have one regional report: For 2018 warnings, every contry will have at least 1 warning, therefore the discounting will apply to regions that have a general warning level 1 and only one aditional regional warning. This to be consistence with regions discounted in previous years (only one regional warning)

##identify which 2018 warnings need to be discounted 
warn2 <- warn_complete %>%
  dplyr::mutate(discount = ifelse(level %in% 1 & warning_count %in% 2, 1, NA)) %>% 
  dplyr::group_by(year, rgn_name) %>% 
  tidyr::fill(discount) %>% 
  dplyr::ungroup() 

##Applying discount accordingly and calculating multiplier for each country each year
warn_discount <- warn2 %>% 
  dplyr::mutate(score =  ifelse(year %in% '2018' & regional %in% 1 & discount %in% 1, score*0.5, score)) %>% 
  dplyr::mutate(score = ifelse(year %in% 2015:2017 & warning_count %in% 1 &regional %in% 1, score*0.5, score)) %>% 
  dplyr::group_by(year, rgn_name) %>% 
  dplyr::summarize(score = mean(score)) %>%
  dplyr::mutate(multiplier = 1- score) %>%
  dplyr::select(year, rgn_name, multiplier) %>%
  dplyr::ungroup() %>% 
  data.frame()
...
data.frame(filter(warn_discount, year==2015)) # checked to make sure I got conversions correct, looks good!


#Add rgn_id
warn_rgn <- name_2_rgn(df_in = warn_discount, 
                       fld_name='rgn_name', 
...
##Compare 2017 multiplier to 2018

old <- warn_rgn %>% 
  dplyr::filter(year== '2017') %>% 
  dplyr::rename(multiplier_2017= multiplier) %>% 
  select(-year)

old_2016 <- warn_rgn %>% 
  dplyr::filter(year== '2016') %>% 
  dplyr::rename(multiplier_2016= multiplier) %>% 
  select(-year)

old_2015 <- warn_rgn %>% 
  dplyr::filter(year== '2015') %>% 
  dplyr::rename(multiplier_2015= multiplier) %>% 
  select(-year)

new <- warn_rgn %>% 
  dplyr::filter(year== '2018') %>% 
  left_join(old, by= 'rgn_id') %>% 
  left_join(old_2016, by= 'rgn_id') %>%
  left_join(old_2015, by= 'rgn_id') %>%
  #filter(!is.na(multiplier_2017)) %>% 
  select(-year) %>% 
  dplyr::rename(multiplier_2018= multiplier)

plot(new$multiplier, new$multiplier_v2017)
abline(0,1, col="red")


compare_18_17 <- ggplot(new, aes(x=multiplier_2018, y= multiplier_2017, label= rgn_id))+
  geom_point()+
  geom_abline(color= 'red')+
  theme_bw()
ggplotly(compare_18_17)

ggplot(new, aes(x=multiplier_2017, y= multiplier_2016))+
  geom_point()+
  geom_abline(color= 'red')+
  theme_bw()

ggplot(new, aes(x=multiplier_2016, y= multiplier_2015))+
  geom_point()+
  geom_abline(color= 'red')+
  theme_bw()
...
abline(0,1, col="red") #Yes! This means data has not been updated. The only difference is that now we are importing the raw data form an excel file instead than a pdf
...
ttci_rgn <- name_2_rgn(df_in = ttcr, 
                       fld_name='country')
