...
warn_raw <- read.csv('raw/tr_travelwarning_2018_raw.csv', stringsAsFactors=FALSE, fileEncoding = "", encoding = 'UTF-8') %>% 
  select(-risk_factor, -location, -date) %>% 
  mutate(level = as.numeric(str_extract(level, '[1,2,3,4]'))) %>% 
  rename(year= assess_year, rgn_name = country)
  #filter(level!= 1)
