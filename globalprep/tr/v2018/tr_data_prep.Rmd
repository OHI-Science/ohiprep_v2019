...
#### 2018 Assesment
Data source updated the way they used to report the data. This year warnings are reported for every country. We input the data manualy to an excel file, converted into csv and wrangle the data to merge with data for previous assesments.

Data was download on 06/28/2018

#### A few notes about getting data: [WORK IN PROGRESS]

Step 1: Copy and paste the data for each contry found in:https://travel.state.gov/content/passports/en/alertswarnings.html

Step 2: Manually chack for every country if there is any regional warning and the risk factor in each case. If different subregions within a country have different warnings, these are put on two different lines, indicating if the warning is regional.
...
Step 3: Import, wrangle and clean the new data so it matches long format of tr_travelwarnings_2017.csv
...
For this assesment, Level 1 is  the status quo warning, therefore regions with a level 1 warning are not considered. This matches better the way the data used to be reported (only countries with risk and not all countries)

```{r}
##Reading and wrangling 2018 data
warn_raw <- read.csv('raw/tr_travelwarning_2018_raw.csv') %>% 
  mutate(country = as.character(country)) %>% 
  mutate(level = as.numeric(str_extract(level, '[1,2,3,4]'))) %>% 
  select(assess_year, level, country , regional) %>% 
  rename(year= assess_year)
 
encodeString(warn_raw$rgn_name)

##Correct names for regions not identified by the name_2_region

#NOTE rgn: Northern Saint-Martin and Guadeloupe and Martinique are reported under "French West Indies". The general warning for this region is 1 but is has a regional regional warning for St. Martin. Therefore, Guadeloupe and Martinique will get a level 1 score and Northern Saint-Martin a level 3 (this is according to the data) and no dicounting for regional.

#MAKE THIS WORK!!!!!!
warn_improved <- warn_raw %>% 
  dplyr::mutate(rgn_name = ifelse(regional == 1 & country == "French West Indies", "Northern Saint-Martin", country)) %>% 
  dplyr::mutate(rgn_name = ifelse(country== "French West Indies" & is.na(regional) & is.na(rgn_name), "Guadeloupe and Martinique", rgn_name)) %>% 
  dplyr::mutate(regional = ifelse(rgn_name == "Northern Saint-Martin", NA, regional)) %>% 
 dplyr::mutate(rgn_name = ifelse(stringr::str_detect(country,"Bonaire"), "#Bonaire", country)) %>%
  dplyr::mutate(rgn_name = ifelse(stringr::str_detect(country,"ROC"), "Republique du Congo", country)) %>% 
  dplyr::mutate(rgn_name = ifelse(stringr::str_detect(country,"Kinshasha"), "Democratic Republic of the Congo", country)) %>% 
  dplyr::mutate(rgn_name = ifelse(stringr::str_detect(country,"Domincan"), "Dominican Republic", country)) %>% 
  dplyr::mutate(rgn_name = ifelse(stringr::str_detect(country,"France"), "France", country)) %>%
    dplyr::mutate(rgn_name = ifelse(stringr::str_detect(country,"Honduras\xca"), "Honduras", country))%>%
  dplyr::mutate(rgn_name = ifelse(stringr::str_detect(country,"Israel"), "Israel", country)) %>%
  dplyr::mutate(rgn_name = ifelse(stringr::str_detect(country,"Sao Tome"), "Sao Tome and Principe", country)) %>%
  dplyr::mutate(rgn_name = ifelse(stringr::str_detect(country,"Turks and Caicos"), "Turks and Caicos Islands", country)) %>%
  dplyr::mutate(rgn_name = ifelse(stringr::str_detect(country,"United Arab Emirates"), "United Arab Emirates", country))
...
warn_old <- read.csv('raw/tr_travelwarnings_2017.csv') %>%
  ...
  filter(!is.na(n)) %>%
  select(-n) %>% 
  #left_join(scores_old, by="category") %>% 
...
##Note: Adjusting scores for regions that only have one regional report: For 2018 warnings, every contry will have at least 1 warning, therefore the if statement changes to:  ifelse(warning_count %in% 2 & regional %in% 1, score*0.5, score). This is becuase countries with only one regional warn will have 2 warn count, the general inform warning plus the regional warning. If a country has more than one regional warn, then warn count would be >= 3 and their score should net be discounted.
## In the past (data from 2015 to 2017): ifelse(warning_count %in% 1 & regional %in% 1, score*0.5, score). This works because not all countries have a warning so whenever there is one warning_count and regional is 1, then then is means that that country has only a one regional warning which should be discouted. 

warn2 <- warn_complete %>%
  mutate(score = ifelse(year == '2018', ifelse(warning_count %in% 2 & regional %in% 1, score*0.5, score), score)) %>% 
...
#Compare old multiliers vs new
warn_multi_old <- read_csv("output/tr_travelwarnings.csv") %>% 
