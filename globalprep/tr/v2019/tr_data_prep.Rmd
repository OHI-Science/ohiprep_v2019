...
##### v2019: Duplicates (Guadeloupe/Martinique, PR/VI, China) - are these being accounted for in the script? (not duplicated in the imported file below)
...
v2019 note - this data doesn't need to be combined, because the travel advisory website has all of the data up to date that covers this entire range between the assessments. (**Skipped this section in the 2019 OHI assessment, can prob delete** )
```{r, eval=FALSE}
...
## v2019: some differences in country names 

write_csv(warn_complete, here("globalprep/tr/v2019/intermediate/warning_complete_1819.csv"))
...
**should we be doing this on warn_improved instead, with just 2019 data??**
```{r, eval=FALSE}
...
from v2018: Next year, consider gapfilling missing territorial regions with administrative country data.
**do we want to do this for v2019? looks like last year's output data has data for all regions** 
...
############################ Identify territories without multiplier data and connect them with multipliers for admin regions 
region_data()
warn_rgn_nas <- warn_rgn_spread %>% 
  filter(is.na(rgn_name)) %>% 
  select(rgn_id, multiplier) %>% 
  left_join(rgns_eez, by = "rgn_id", "rgn_name") %>% 
  select(rgn_id, multiplier, rgn_name, admin_rgn_id, admin_country_name)

admin_rgn_multipliers <- warn_rgn_nas %>% 
  select(rgn_id = admin_rgn_id, admin_country_name) %>% 
...
write_csv(warn_rgn_all_rgns, here('globalprep/tr/v2019/output/tr_travelwarnings_2019.csv'))

## Create gapfill file. No gapfill in this case so gapfill = 0

travelwarning_gf <- read.csv(here("globalprep/tr/v2019/output/tr_travelwarnings.csv")) %>% 
  mutate(multiplier = 0) %>% 
  rename(gapfilled = multiplier)

write_csv(travelwarning_gf, here('globalprep/tr/v2019/output/tr_travlewarnings_gf.csv'))
