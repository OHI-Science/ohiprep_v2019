...
These data are from the World Travel & Tourism Council (http://www.wttc.org/).  We use "direct" employment data (eee mazu: globalprep/_raw_data/WTTC/d2018/README.md for instructions on obtaining data). The data extend to 2028, but these values are projections.  The actual data goes to 2019.
...
##### v2019: Need to account for Côte d'Ivoire, + duplicates (Guadeloupe/Martinique, PR/VI, China) + didn't create a new file? see notes written at the end of the function script! 
...
tr_data_raw <- rgn_names %>%
    full_join(tr_jobs_pct_tour %>%
              ...
              by = c('rgn_id', 'year')) 
# v2018: Some regions (rgn_ids 32, 52 and 140 in the 2018 assesment) appear to have an error bteween some years, Ep value > 100%. This line makes this values NA
## v2019: Should we be tracking NAs here? 
...
## 262 NaNs - should we be concerned about this? 
...
### v2019: guadeloupe and martinique being reported together and separately; need to compress? 
...
### not sure this is needed for v2019?
...
warn_old_no_subs$country <- as.character(warn_old_no_subs$country)
