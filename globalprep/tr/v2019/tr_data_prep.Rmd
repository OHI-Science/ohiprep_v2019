...
warn_raw <- warn_raw %>% 
  ...
  filter(!(regional == 1 & n >1)) %>% # remove regions that have regional warnings in addition to country wide warnings, but retain those where country warning level is based on regional information
  select(assess_year, level, country) %>% 
  rename(year= assess_year)

test <- count(warn_raw, country)
test2 <- warn_raw %>% 
