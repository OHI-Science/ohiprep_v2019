...
# make sure all the uninhabited regions are NA
##### write code that detects whether a region is in the low_pop df, and if it is, assign a value of NA to Ep column . Something like this: 

tr_data_raw_gf2 <- tr_data_raw_gf %>% 
...
################# 245 NAs - this is because all of the regions under "Southern Islands" are not being gapfilled because there are no data for any of these regions to base regional gapfilling off of. Need to decide whether we will leave these as NAs, gapfill on the r0 (world) scale, or use administrative region data.
