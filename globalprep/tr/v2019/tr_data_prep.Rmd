...
## Travel warnings
Previously, US state department data only were used to identify travel warnings for each country. In 2019 we incorporated advisories from the Canadian government to fill in data for countries the US did not establish warnings for. The Canadian advisories were matched to the numeric scale used by the US State Department since 2018 (1-4, ranges from level 1 (normal precautions) to level 4 (do not travel)). 
...
* Travel warnings for 2018-2019 (downloaded from U.S State Department and Canadian Government on 07/02/2019)
...
### Delete this whole step for v2019??

Join new data with previous years' assesment data
...
## v2019: some differences in country names since we imported the raw data that hadn't been mutated yet
...
warn_rgn_spread <- warn_rgn %>%
  ...
  ########## ^ remove this and instead fill in admin data for territories/regions with no warnings 
  filter(rgn_id <= 250) %>%
...
######## Do we want to change this since we gapfilled for territorial regions? 
