...
ggplotly(ggplot(fmi_gdp2, aes(x = gdppcppp, y = fmi, labels = rgn_id)) +
  geom_point() +
  geom_abline(slope = 1, intercept = 0, color = "red"))
...
ggplotly(ggplot(fmi_ao2, aes(x = ao_need_score, y = fmi, labels = rgn_id)) +
  geom_point() +
  geom_abline(slope = 1, intercept = 0, color = "red"))
...
# p <0.001, reject null that true correlation=0
# cor = ~0.66 (decent strong positve)
...
ggplotly(ggplot(fmi_spi2, aes(x = resilience_score, y = fmi, labels = rgn_id)) +
  geom_point() +
  geom_abline(slope = 1, intercept = 0, color = "red"))
...
# p <0.001, reject null that true correlation=0
# cor = ~0.64 (decent strong positve)
...
ggplotly(ggplot(fmi_wgi2, aes(x = resilience_score, y = fmi, labels = rgn_id)) +
  geom_point() +
  geom_abline(slope = 1, intercept = 0, color = "red"))

fmi_wgi_plot <- fmi_wgi2 %>% 
  select(fmi, resilience_score)
...
# save the cleaned gdppc data for existing regions
...
# Assign georegion labels to each region for each level (r0, r1, r2)
fmi_rgn_gf <- data.frame(year=min(fmi_rgn$year):max(fmi_rgn$year)) %>% 
  merge(UNgeorgn, by=NULL) %>% 
  rename(rgn_name = rgn_label)

# Combine the two data frames by region id and year
# Calculate means across increasing geopolitical levels (e.g. r2, r1), using the highest resolution possible
fmi_rgn_gf <- fmi_rgn_gf %>%  
  left_join(fmi_rgn, by = c("rgn_id", "rgn_name", "year")) %>%
  group_by(r2_label, year) %>%
  mutate(r2_value = mean(fmi, na.rm=TRUE)) %>%
  ungroup() %>%
  group_by(r1_label, year) %>%
  mutate(r1_value = mean(fmi, na.rm=TRUE)) %>%
  ungroup() %>%
  group_by(r0_label, year) %>%
  mutate(r0_value = mean(fmi, na.rm=TRUE)) %>%
  ungroup() %>% 
  filter(year == 2018)
summary(fmi_rgn_gf) 
...
# For `score` cells that still have NA values (still several hundred):
# Check to see if r2 has a value, if so use that to gapfill `fmi`, otherwise use r1, otherwise use r0
fmi_rgn_gf2 <- fmi_rgn_gf %>%
  mutate(gapfilled = ifelse(is.na(fmi) & !is.na(r2_value), 1, 0)) %>% # create binary gapfilled column
  mutate(method = ifelse(is.na(fmi) & !is.na(r2_value), "UN_geopolitical region avg, r2", NA)) %>%
  mutate(fmi = ifelse(is.na(fmi), r2_value, fmi)) %>%
  mutate(gapfilled = ifelse(is.na(fmi) & !is.na(r1_value), "1", gapfilled)) %>%
  mutate(method = ifelse(is.na(fmi) & !is.na(r1_value), "UN_geopolitical region avg, r1", method)) %>%
  mutate(fmi = ifelse(is.na(fmi), r1_value, fmi)) %>%
  mutate(gapfilled = ifelse(is.na(fmi) & !is.na(r0_value), "1", gapfilled)) %>%
  mutate(method = ifelse(is.na(fmi) & !is.na(r0_value), "UN_geopolitical region avg, r0", method)) %>%
  mutate(fmi = ifelse(is.na(fmi), r0_value, fmi))
  

fmi_rgn_gf2$fmi[fmi_rgn_gf2$rgn_id %in% low_pop_vector] <- NA
#FMI should now be NA for low popuation regions

summary(fmi_rgn_gf2) # 20 NAs in FMI column, as expected after applying the low_pop vector
```
