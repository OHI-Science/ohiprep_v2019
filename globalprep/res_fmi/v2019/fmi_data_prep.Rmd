...
d_stand_rgn <- read_csv(here("globalprep/res_fmi/v2019/intermediate/gdppcppp_ohi.csv"))
...
mod12 <- lm(fmi~spi + r0_label, data = all_layers)
...
fmi_gf <- UNgeorgn %>% 
  ...
  select(-rgn_name, spi=resilience_score)

# Gapfill spi scores for regions that are NA (might end up doing this later in res_spi v2019) using mean of all spi values
fmi_gf <- fmi_gf %>% 
  mutate(spi = ifelse(str_detect(r2_label, "Southern Islands"), mean(spi, na.rm=TRUE), spi)) 
...
fmi_gf2 <- fmi_gf %>%
  ...
  dplyr::do({ 
    ...
    data.frame(., fmi_pred1) # do loop applies the model fitting and prediction to each country group
  }) %>% 
  dplyr::ungroup()
