...
#ico_rgn_id column values being preserved in this csv, but not where it is being imported below. Converting the column to numeric didn't solve the problem!
# Changed to write.csv instead of write_csv, because write_csv is coercing ico_rgn_id column to logical instead of numeric/integer format
# Might need an update to the tidyverse to solve this problem since read/write_csv seems to be causing the issue
...
# commenting this out for now to prevent the data loss described above
...
ico_spp_rgn <- ico_spp_rgn_prepped %>%  #read_csv(file.path(dir_github, 'int/ico_spp_rgn_prepped.csv')) %>%
...
ico_spp_rgn_cat <- ico_spp_rgn_cat %>%
  ...
  filter(ico_gl | ico_rgn_id == rgn_id) %>% # keep (all globally iconic) and (regionally iconic in region only) - the whole ico_rgn_id column is NAs
  distinct()
...
# commenting this out for now bc ico_rgn_id column is being lost!
