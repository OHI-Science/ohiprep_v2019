...
ico_spp_countries <- mc_get_from_api(url = ico_country_url, param_vec = ico_list$iucn_sid, api_key = api_key, delay = 0.5)
...
# 2019 version removed rgn_iucn2ohi file; replaced with name_2_rgn function, code below now becomes unnecessary (can delete later)
...
## error check on region name matching; landlocked --> not matched (597 in v2019)
...
# In 2019 non_match has 0 obs. (no NAs)

# I think this step is unncessary now that we used the name_2_rgn function? OHI rgn_name already called rgn_name
ico_spp_rgn_raw <- ico_spp_rgn_raw %>%
#  rename(rgn_name = ohi_rgn_name) %>%
  select(-country) %>% # removed "country" instead of iucn_rgn_name
  filter(!is.na(rgn_id)) %>%
  distinct()

summary(ico_spp_rgn_raw)
...
ico_spp_rgn_prepped <- ico_spp_rgn_raw %>%
  left_join(ico_list, by = 'iucn_sid') # need to see what this was in the old file
...
write_csv(ico_spp_rgn_prepped, file.path(dir_github, 'int/ico_spp_rgn_prepped.csv'))

ico_spp_rgn_prepped <- ico_spp_rgn_prepped %>% 
  select(iucn_sid, sciname, comname, presence, origin, distribution_code, cat, ico_gl, rgn_id, rgn_name, subpop, ico_rgn_id, param_id)

# v2019: 2018 version had more amendments to ico_spp_rgn_prepped after saving it - should I move this around? 
# v2019: took out api_error as it is no longer present in the data set. In 2018 all the values in this column were NA

# v2019: running these lines returned: 
...
ico_spp_rgn <- read_csv(file.path(dir_github, 'int/ico_spp_rgn_prepped.csv')) %>%
...
status_trend <- ico_sum %>% # status and trend in this year's assessment
  filter(year == max(year)-2) %>%  # max year of ico_sum is 2019
...
ico_compare <- read_csv(file.path(dir_previous, 'summary/ico_summary.csv')) %>% 
  filter(year == max(year)) %>% # max year of v2019 ico_summary is 2019
