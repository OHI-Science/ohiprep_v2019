...
get_from_api <- function(url, param) {
    api_info_json <- fromJSON(sprintf(url, param, api_key)) 
    len_result <- api_info_json %>% .[[2]] %>% length()
    df_len_zero <- data.frame(name=as.character(param), 
                              result.year=NA, result.code=NA, result.category=NA, 
                              stringsAsFactors = FALSE)
    if(len_result==0){api_info = df_len_zero}else{api_info = api_info_json %>% 
      data.frame(stringsAsFactors = FALSE)}
} 
...
ico_list_raw <- read_csv(file.path(dir_goal, 'raw/ico_list_raw.csv'))
...
ico_spp_countries <- mc_get_from_api(ico_country_url, ico_list$iucn_sid) ### run line by line or get stuck here...
...
ico_assess_raw <- mc_get_from_api(ico_past_assess_url, ico_list$iucn_sid) ## run line by line not entire chunk otherwise gets stuck here...
...
ico_assess_full <- ico_assess %>%
  ...
  group_by(iucn_sid) %>%
  fill(cat, cat_txt, cat_score) %>% ## fills all the way to latest year
  ungroup()
...
`r DT::datatable(ico_status_raw %>% filter(year == 2017))`

### Iconic Species processed status and trend by region

`r DT::datatable(ico_status_calc %>% filter(year == 2017), caption = 'ICO status')`

`r DT::datatable(ico_trend %>% filter(year == 2017) %>% select(-trend_lm), caption = 'ICO trend')`
...
# ico_previous_status <- read_csv(file.path(dir_previous, "output/ico_status_calc.csv")) %>% 
#   filter(year == 2016)
# ico_compare <- ico_status_calc %>%
#   filter(year == 2016) %>% 
#   dplyr::full_join(ico_previous_status, by = "rgn_id")

# ico_prev <- list.files(sprintf('~/github/ohi-global/global%s/', previous_scen), pattern = 'final_formatted_scores',  full.names = TRUE) %>% 
#   read_csv() %>% 
#   filter(scenario == 2016, goal == 'ICO', dimension %in% c('status', 'trend')) %>% 
#   spread(key = dimension, value = score)

status_trend <- ico_sum %>% 
  filter(year == max(year)-1) %>% 
  select(rgn_id, ico_status, ico_trend, n_spp) %>% 
  distinct() ## status and trend in this year's assessment
...
ico_compare <- read_csv(file.path(dir_previous, 'summary/ico_summary.csv')) %>% 
  ...
  rename(new_status = ico_status, new_trend = ico_trend, new_nspp = n_spp)

compare_status_plot <- ggplot(ico_compare) + 
  geom_point(aes(previous_status, new_status, color = rgn_id)) + 
  labs(x = sprintf('ICO status %s', scenario),
       y = sprintf('ICO status v%s', previous_scen),
       title = 'ICO status comparison')
ggplotly(compare_status_plot)

trend_compare_plot <- ggplot(ico_compare, aes(x = new_trend, y = previous_trend, color = rgn_id)) +
  geom_point(alpha = .6) +
  geom_abline(slope = 1, intercept = 0, color = 'red') +
  labs(x = sprintf('ICO trend %s', scenario),
       y = sprintf('ICO trend v%s', previous_scen),
       title = 'ICO status comparison')
