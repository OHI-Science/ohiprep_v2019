...
v6_saltmarsh_pts$geometry1 <- v6_saltmarsh_pts$geometry

v6_saltmarsh_pts <- st_zm(v6_saltmarsh_pts)

#for some reason there are 0's attached to the end of the geometry column, lets remove those.
geometry_df = data.frame(substr(v6_saltmarsh_pts$geometry1, 1, nchar(v6_saltmarsh_pts$geometry1)-4))

##Separate geometry DF into two columns: 
geometry_df <- separate(geometry_df, )
...
compare_hab_new_data <- compare_habitat_extent %>%
  filter(km2.x > 0, is.na(year.y)) ## 53 new regions
