...
This script generates the extent of salt marsh for each OHI region. 
...
***
# Methods
Reclassify the saltmarsh extent data into a mask of 1 or NA, and then compute zonal statistics for the count of cells within an OHI region that have seagrass and then convert into km2.
...
#for some reason there are 0's attached to the end of the geometry column, lets remove those.
#v6_saltmarsh_pts$geometry = substr(v6_saltmarsh_pts$geometry, 1, nchar(v6_saltmarsh_pts$geometry)-4)
## Now need to add a parenthesis to the end of each geometry since we just removed that 
#v6_saltmarsh_pts$geometry = paste(v6_saltmarsh_pts$geometry,")", sep = "")
## Need to set geometry again since we tampered with the data inside of it
#sfc <- v6_saltmarsh_pts$geometry
#st_multipoint(sfc)
#st_geometry(sfc)
#st_geometry(v6_saltmarsh_pts$geometry) 
...
fasterize_py <- fasterize::fasterize(v6_saltmarsh_py_moll, raster = zones, 
                                     field = NULL)
...
combined_values_2 <- getValues(combined_seagrass1)
