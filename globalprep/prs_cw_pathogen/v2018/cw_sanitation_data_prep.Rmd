...
Get an idea of what needs to be gapfilled.
...
For this year assesment, this gapfill will be relevant for Gibraltar (rgn_id 60) and Wester Sahara (rg_id 63).
...
sani_georgn_gf <- sani_georgn_gf %>%
  mutate(basic_sani_georgn = ifelse(is.na(basic_sani_prop), basic_sani_r2, basic_sani_prop)) %>%
  mutate(basic_sani_georgn = ifelse(is.na(basic_sani_georgn), basic_sani_r1, basic_sani_georgn)) %>% 
  mutate(gapfilled = ifelse(is.na(basic_sani_prop) & !is.na(basic_sani_georgn), "gapfilled", NA)) %>%
  mutate(method = ifelse(is.na(basic_sani_prop) & !is.na(basic_sani_r2), "UN georegion (r2)", NA)) %>%
  mutate(method = ifelse(is.na(basic_sani_prop) & is.na(basic_sani_r2) & !is.na(basic_sani_r1), "UN georegion (r1)", method))


##Filter regions that were gapfilled with georegion method
rgn_gf2 <- sani_georgn_gf %>%
  filter(gf_count == years_data)
...
```{r}
sani_complete <- sani_gf_lm %>%
  full_join(rgn_gf2, by = NULL) %>% 
  arrange(rgn_id, year) %>% 
  mutate(basic_sani_all= ifelse(is.na(basic_sani_prop),basic_sani_georgn,  basic_sani_prop)) %>% 
  select(rgn_id, rgn_name, year,basic_sani_all, method)

#Save data
write.csv(sani_complete, "intermediate/sani_complete_gf.csv", row.names=FALSE)
