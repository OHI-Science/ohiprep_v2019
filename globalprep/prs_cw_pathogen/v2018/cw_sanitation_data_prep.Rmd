...
# Some regions have no data, let's filter those out
...
sani_gf_lm <- sani_gf_lm %>%
  ...
  select(rgn_id, rgn_name, year, basic_sani_prop, gf_count)

```
#Gapfilling 2
Georegional gapfilling for regions that do not have data

Note to self: Haven't run this code but it is spotential way to gapfill by georegions (based on `tr_data proep`)
This gapfill will be relevant for Gibraltar rgn_id 60 and Wester Sahara rg_id 63
...
sani_georgn_gf <- sani_georgn_gf %>%
  mutate(basic_sani_all = ifelse(is.na(basic_sani_prop), basic_sani_r2, basic_sani_prop)) %>%
  mutate(basic_sani_all = ifelse(is.na(basic_sani_all), basic_sani_r1, basic_sani_all)) %>% 
  mutate(gapfilled = ifelse(is.na(basic_sani_prop) & !is.na(basic_sani_all), "gapfilled", NA)) %>%
...
sani_complete <- sani_georgn_gf %>% 
  select(rgn_id, year, basic_sani_all) %>% 
  arrange(rgn_id, year)
