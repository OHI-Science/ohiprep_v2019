...
sani <- sani_raw %>%
  select(country = V1,
         ...
         pop = V4,
         improve_pct = V6) %>%
  mutate(pop = str_remove(pop, pattern = " ")) %>%
  mutate_at(.vars = c("pop", "improve_pct"), .funs = as.numeric)%>%
  mutate(pop = pop * 1000,
          basic_tot = improve_pct/100 * pop)
  
```

Select and scale percentage of population with access to improved sanitation ()

```{r}
sani_improved <- sani %>%
   select(country, year, access_pct = improve_pct) %>%
...
#Note t self: Figure out what to do with regions that havr funny cahracters and are not able to be read by the function
```
