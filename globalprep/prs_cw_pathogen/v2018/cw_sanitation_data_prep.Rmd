...
##Identify inhabitaed regions according to table in github
...
filter(sani_georgn_gf, !is.na(inhab)) %>% 
  ...
  unique() %>%
  data.frame() #NOTE: Some of the inhabited regions have been gapfilled!


#Fill in all inhabited in rgn_inhab with max sanitation prop
sani_complete <- sani_georgn_gf %>% 
  mutate(basic_sani_prop = ifelse(is.na(inhab), basic_sani_prop, 1)) %>% 
  select(rgn_id, year, basic_sani_prop, method)
...
#Note: there is a difference between the list of islands with no population in the mar_pop_25mi dataset witht the inhabit list in github. In the mar_pop_25mi, rgn_id 144 - Jan Mayen is not identified as a pop_0 region while in the github list it is. Population data also considers Antartica (rgn_id213)
...
#Calculate refernce point
ref_calc <- unsani_pop %>% 
  filter(year %in% 2000:2009) %>% #years of reference
  summarise(ref= quantile(propWO_x_pop_log, probs=c(0.99), na.rm = T)) %>% 
  .$ref
...
unsani_prs <- unsani_pop %>%
  ...
  select(rgn_id, year, pressure_score) #limits pressure scores not to be higher than 1
...
```{r trend}
##NEEDS A LOOP TO CALCULATE THE TREND?

#Min and max year of data to calculte trend
maxyear <- max(unsani_prs$year)
minyear <- maxyear-4

unsani_prs_mdl <- unsani_prs %>%
    filter(!is.na(pressure_score)) %>% # lm can't handle NAs!!
    group_by(rgn_id) %>%
    do(mdl = lm(pressure_score ~ year, data=.),
       adjust_trend = .$pressure_score[.$year == minyear]) %>%
...
summary(unsani_prs_mdl)

#Save trend data
#write.csv(unsani_prs_mdl, "output/po_pathogen_popdensity25mi_trend.csv", row.names=FALSE)
```
