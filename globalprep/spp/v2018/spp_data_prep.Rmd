...
mean_risk_comp <- raster::raster("globalprep/spp/v2018/_output/mean_risk_raster_comp.tif")

n_comp <- raster::raster("globalprep/spp/v2018/_output/n_spp_risk_raster_comp.tif")
...
trend_comp <- raster::raster("globalprep/spp/v2018/_output/trend_raster_comp.tif")

n_trend_comp <- raster::raster("globalprep/spp/v2018/_output/n_trend_raster_comp.tif")
...
```{r}
status <- read.csv("globalprep/spp/v2018/output/sp_status_global.csv") %>%
...
write.csv(status, "globalprep/spp/v2018/output/sp_status_global_gf.csv", row.names=FALSE)          

trend <- read.csv("globalprep/spp/v2018/output/sp_trend_global.csv") %>%
  mutate(gapfilled = ifelse(rgn_id == 232, 1, 0)) %>%
  mutate(method = ifelse(rgn_id == 232, "mean of neighbors", NA)) %>%
  dplyr::select(rgn_id, gapfilled, method)
write.csv(trend, "globalprep/spp/v2018/output/sp_trend_global_gf.csv", row.names=FALSE)          
...
trend_df <- raster::zonal(trend_stack, rgns, fun='sum', progress="text")
...
```{r}
res <- read.csv("globalprep/spp/v2018/output/sp_status_3nm.csv") %>%
  mutate(gapfilled = ifelse(rgn_id == 19, 1, 0)) %>%
  mutate(method = ifelse(rgn_id == 19, "eez scale data used", NA)) %>%
  dplyr::select(rgn_id, year, gapfilled, method)
write.csv(res, "globalprep/spp/v2018/output/sp_status_3nm_gf.csv", row.names=FALSE)          
