...
Added Casey Ohara's personal common.R functions into the common_fxns.R file located in the '_setup' file for this goal. 
...
write.csv(status, here("globalprep/spp/v2019/_output/sp_status_global.csv"), row.names=FALSE)  
...
```{r}
status <- read_csv(here("globalprep/spp/v2019/_output/sp_status_global.csv"))
...
status <- read.csv(here("globalprep/spp/v2019/_output/sp_status_global.csv")) %>%
...
write.csv(trend, here("globalprep/spp/v2019/_output/sp_trend_global.csv"), row.names=FALSE)  
...
trend <- read.csv(here("globalprep/spp/v2019/_output/sp_trend_global.csv"))  
...
write.csv(status, here("globalprep/spp/v2019/_output/sp_status_global_gf.csv"), row.names=FALSE)          

trend <- read.csv(here("globalprep/spp/v2019/_output/sp_trend_global.csv")) %>%
  mutate(gapfilled = ifelse(rgn_id == 232, 1, 0)) %>%
  mutate(method = ifelse(rgn_id == 232, "mean of neighbors", NA)) %>%
  dplyr::select(rgn_id, gapfilled, method)
write.csv(trend, here("globalprep/spp/v2019/_output/sp_trend_global_gf.csv"), row.names=FALSE)          
...
# get eez value:
eez_status <- read.csv(here("globalprep/spp/v2019/_output/sp_status_global.csv")) %>%
  filter(rgn_id == 19) %>%
...
write.csv(resilience, here("globalprep/spp/v2019/_output/sp_status_3nm.csv"), row.names=FALSE)  

```

### Resilience: save a record of gapfilling
```{r}
res <- read.csv(here("globalprep/spp/v2019/_output/sp_status_3nm.csv")) %>%
  mutate(gapfilled = ifelse(rgn_id == 19, 1, 0)) %>%
  mutate(method = ifelse(rgn_id == 19, "eez scale data used", NA)) %>%
  dplyr::select(rgn_id, year, gapfilled, method)
write.csv(res, here("globalprep/spp/v2019/_output/sp_status_3nm_gf.csv"), row.names=FALSE)          
...
status <- read.csv(here("globalprep/spp/v2019/_output/sp_status_3nm.csv")) %>%
