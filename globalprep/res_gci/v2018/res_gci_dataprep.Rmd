...
wef_gf <- wef_gf %>%
  mutate(gapfill = ifelse(N < N_max, 1, 0)) %>%
  mutate(method = ifelse(N == 1, "within region: one value all years", NA)) %>%
  mutate(method = ifelse(N < N_max & N > 1, "within region: lm", method)) %>%
  mutate(score = ifelse(N == 1, gf_mean, score)) %>%
  mutate(score = ifelse(N < N_max & N > 1, gf_lm, score)) %>%
...
wef_rgn_gf_un <- georegions %>% # georegions sourced from common.R
  merge(years) %>%
  left_join(wef_rgn) # joining adds in missing regions; will be gapfilled by methods below
...
## running this line by line is helpful
wef_gf2 <- wef_rgn_gf_un %>%
  ...
  mutate(method = ifelse((is.na(score) & is.na(score_pred_r2) & !is.na(score_pred_r1)), "UN r1 georegion", method)) %>% # helpful to view here
...
unique(wef_gf2[is.na(wef_gf2$score),"rgn_id"]) # regions missing scores
```
...
unique(res_data[is.na(res_data$resilience_score),"rgn_id"]) # check; should be all and only uninhabited regions
