...
Read in the polygons from the WDPA dataset; filter as needed; reorder.
...
gdal_rast2 <- function(src, rast_base, dst = NULL, value = NULL, override_p4s = FALSE) {
  ...
  if(is.null(value)) { ### default: choose first numeric column as value
    ...
    if(sum(num_cols) == 0) {
      message('No numeric or integer column found in source shapefile')
      stop()
    } else {
...
rast_wdpa_file   <- file.path(dir_goal_anx, 'rast', 'wdpa_2018_moll_500m_gdal.tif')

if(!file.exists(rast_wdpa_file)) {
  shp_xformed_file <- file.path(dir_data, 'shps', 'WDPA_June2018_shp_xformed')
  
  ## don't need to change file path name from year to year
