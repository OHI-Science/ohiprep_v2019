---
title: 'OHI: Lasting Special Places v2018'
...
status_3nm_old <- read_csv(file.path(dir_goal, '../v2016/output', 'lsp_protected_offshore3nm.csv')) %>%
  full_join(read_csv(file.path(dir_goal, 'output', 'rgn_area_offshore3nm.csv')),
            by = 'rgn_id') %>%
...
status_1km_old <- read_csv(file.path(dir_goal, '../v2016/output', 'lsp_protected_inland1km.csv')) %>%
  full_join(read_csv(file.path(dir_goal, 'output', 'rgn_area_inland1km.csv')),
            by = 'rgn_id') %>%
...
ggsave(file.path(dir_goal, 'int/plot_v2017_v2018.png'), 
...
#  88 Cook Islands 0 -> 100
# 146 Qatar               0 -> 71  OK?: looks like land-based protected area; not in 2016 raster
...
checks <- mpa2018 %>% filter(rgn_id %in% c(153, 190)) %>% filter(year > 2000) %>% rename(prot_3nm_2018 = a_prot_3nm) %>%
  left_join(tpa2018 %>% filter(rgn_id %in% c(153, 190)) %>% filter(year > 2000) %>% rename(prot_1km_2018 = a_prot_1km),
            by = c('rgn_id', 'year')) %>%
  left_join(mpa2017 %>% filter(rgn_id %in% c(153, 190)) %>% filter(year > 2000) %>% rename(prot_3nm_2017 = a_prot_3nm),
            by = c('rgn_id', 'year')) %>%
  left_join(tpa2017 %>% filter(rgn_id %in% c(153, 190)) %>% filter(year > 2000) %>% rename(prot_1km_2017 = a_prot_1km),
            by = c('rgn_id', 'year')) %>%
  left_join(marea2018 %>% filter(rgn_id %in% c(153, 190)) %>% rename(marea18 = area),
            by = c('rgn_id')) %>%
  left_join(tarea2018 %>% filter(rgn_id %in% c(153, 190)) %>% rename(tarea18 = area),
            by = c('rgn_id')) %>%
  left_join(marea2017 %>% filter(rgn_id %in% c(153, 190)) %>% rename(marea17 = area),
            by = c('rgn_id')) %>%
  left_join(tarea2017 %>% filter(rgn_id %in% c(153, 190)) %>% rename(tarea17 = area),
            by = c('rgn_id'))
```
