...
Same as previous assessment other than updated data from April 2019. 
...
library(gdalUtils)
library(tidyverse)
source('~/github/ohiprep_v2019/workflow/R/common.R')

goal     <- 'globalprep/lsp'
scenario <- 'v2019'
dir_goal_anx   <- file.path(dir_M, 'git-annex', goal, scenario) 
dir_goal  <- file.path('~/github/ohiprep_v2019', goal, scenario)
...
library(foreign)
read.dbf(file.path(dir_data, 'WDPA_Apr2019-shapefile', 'WDPA_Apr2019-shapefile-polygons.dbf'))
## check to see if .dbf actually uploaded in mazu, it did. 222,015 rows. 
...
if(!file.exists(paste0(shp_reorder, '.shp'))) {
  ...
  ## Read in the raw shapefile (4.1 GB); 468 sec on Mazu
...
#library(sf)
read_sf(file.path(dir_data, 'shps', 'WDPA_Apr2019_shp_ordered.shp'))
...
read_sf(file.path(dir_data, 'shps', 'WDPA_Apr2019_shp_xformed.shp'))
#check to see if it worked!
```
