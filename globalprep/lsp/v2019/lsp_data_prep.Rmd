...
##Above loop would not work. Looking through the 2018 LSP issue, Iwen used the following code to run each lsp_crosstab() individually, rather than using mclapply. 
...
#region_data() #use this function to call rgns_eez, which is called below
...
### Protected areas (3 nm offshore, 2019 only):

`r DT::datatable(prot_3nm %>% filter(year == 2019) %>% select(-year, -contains('cell')), caption = '3 nautical mile offshore zone - area in km^2^')`

### Protected areas (1 km inland, 2019 only):

`r DT::datatable(prot_1km %>% filter(year == 2019) %>% select(-year, -contains('cell')), caption = '1 kilometer inland zone - area in km^2^')`

### Protected areas (full EEZ, 2018 only):

`r DT::datatable(prot_eez %>% filter(year == 2019) %>% select(-year, -contains('cell')), caption = 'Full EEZ - area in km^2^')`
