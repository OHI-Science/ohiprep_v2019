...
source(here('workflow/R/common.R')) # keeps losing UNgeorgn() after running add_georegion_id()?? - might be getting overwritten by df named UNgeorgn 
...
###### This is currently gapfilling using already-gapfilled data. Thinking about writing some lines into the function script that prevents this from happening
...
Comparing against last year's data. For example, compare the data for 1976-2014 for the 2018 and 2019 assessment year. Do not need to look at new years of data (2015-2016), which is only available in the 2019 assessment year, since those did not exist in the previous year's assessment.
...
#################### Delete most of these, leave a few examples 

## Malta
new <- read.csv(here("globalprep/np/v2019/int/tonnes.csv")) %>%
  filter(rgn_id==68, year %in% c(1990:2014))
...
## Cayman Islands
new <- read.csv(here("globalprep/np/v2019/int/tonnes.csv")) %>% 
  filter(rgn_id==113, year %in% c(1976:2014))
old <- read.csv(here('globalprep/np/v2018/int/tonnes.csv')) %>%  # CHANGE YEAR
  filter(rgn_id==113, year %in% c(1976:2014)) %>% 
  rename(tonnes_old = tonnes)
...
## Curacao Islands
new <- read.csv(here('globalprep/np/v2019/int/tonnes.csv')) %>% 
  filter(rgn_id==244, year %in% c(1976:2014))
old <- read.csv(here("globalprep/np/v2018/int/tonnes.csv")) %>%  # CHANGE YEAR
  filter(rgn_id==244, year %in% c(1976:2014)) %>% 
  rename(tonnes_old = tonnes)
