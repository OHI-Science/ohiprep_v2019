...
new_libya <- filter(new, rgn_id==67) %>%
  arrange(commodity, year) %>% 
  rename(tonnes_new = "tonnes")
old_libya <- filter(old, rgn_id==67) %>%
  arrange(commodity, year)
...
# Looks like there may have been a mistake made last year because np_harvest_tonnes had NA for shells in 2013-2015, where same data file for v2019 had 3-5 tons for this product and 0 - 1 for previous years. Odd because the raw data have matching numbers for tonnes, so maybe something got messed up in last year's gapfilling procedure. 
...
# weird because it doesnt look like anything is changing at all??? need to look into this more (to do)
