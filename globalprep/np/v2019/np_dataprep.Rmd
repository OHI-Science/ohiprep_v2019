...
for (f in files){ # f = files[1]
  ...
  # v2018 unmatched: eswatini, palestine, yugoslavia (all of these are landlocked)  
  ...
  m_rgn <- m_rgn %>% # need to double check that this is doing what I want it to be doing 
    ...
    summarize(tonnes = sum_function(tonnes)) %>%
    ungroup()

  ## units: rename value field to units based on filename
  names(m_rgn)[names(m_rgn) == 'value'] <- units  
  
  ## output to .csv - can probably compress this into one line? 
  harvest_out <- sprintf(here('globalprep/np/v2019/int/%s.csv'), units)
  write.csv(m_rgn, harvest_out, row.names = FALSE, na = '')
  
  # compressed into one line (added tidyverse to libraries at the top): 
  write_csv(m_rgn, file.path(here('globalprep/np/v2019/int/tonnes.csv')))
}
...
```{r}
############ Change back to 2019 once I get the correct data! 

h_tonnes <- read.csv(here('globalprep/np/v2018/int/tonnes.csv'))

h_usd <- read.csv(here('globalprep/np/v2018/int/usd.csv'))
...
# v2019: tonnes and usd values are the same in this df - is this correct? 
...
# not finding this function; went into np_fxn.R file to investigate and the file it's trying to pull from isn't in the ohi-global/eez/layers folder 

################################

h <- h %>% np_regr_fill(years_back = 10, vars = 'td', scope = 'rgn_id')
h <- h %>% np_regr_fill(vars = 'tdy', scope = 'georgn_id')
################# error here: column 'georgn_id' is unknown
...
## Check a random country and commodity - what are we looking for exactly? 
...
old <- read.csv('globalprep/np/v2016/int/tonnes.csv') %>%  # CHANGE YEAR
  filter(rgn_id==113, year %in% c(1976:2015)) %>% 
  rename(tonnes_old = tonnes)
...
old_tonnes <- read.csv(here("globalprep/np/v2016/output/np_harvest_tonnes.csv")) %>% 
  filter(rgn_id==6, year %in% c(2010:2013)) %>% 
  rename(tonnes_old = tonnes)

compare <- old_tonnes %>% 
full_join(new_tonnes, by = c("rgn_id","product","year"))
#### some changes here in 2012-2013
...
old <- read.csv(here("globalprep/np/v2016/output/np_harvest_tonnes.csv")) %>% 
  filter(rgn_id==61, year %in% c(2010:2013)) %>% 
  rename(tonnes_old = tonnes)
...
smooth_new <- read.csv(here("globalprep/np/v2019/int/np_harvest_smoothed_data.csv")) %>% 
  ...
  select(rgn_id, product, year, tonnes, tonnes_rel)
smooth_old <- read.csv(here("globalprep/np/v2016/int/np_harvest_smoothed_data.csv")) %>% 
  filter(rgn_id==112, year %in% c(2010:2013)) %>% 
...
old <- read.csv('globalprep/np/v2016/output/np_harvest_tonnes.csv') # CHANGE YEAR
