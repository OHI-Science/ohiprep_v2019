...
New year of FAO data (1976-2016), but no changes to general methods.
...
FAO metadata found [here](http://ref.data.fao.org/dataset?entryId=aea93578-9b01-4448-9305-917348ca00b2&tab=metadata) # this link not working
 
**Downloaded**: June 20 2019
...
dir_fao_data <- file.path(dir_M, 'git-annex/globalprep/_raw_data/FAO_commodities/d2019') # creates a directory to where files are stored
...
# checks names syntactically, strips leading and trailing whitespace, prevents conversion of characters to factors 
  ...
  ## v2018: Some changes to region names that aren't being detected by name_2_rgn below
  m <- m %>%
  ...
  # v2019 duplicates: china, HK SAR, Macao SAR, Guadeloupe, etc - these are addressed below in the group_by/summarize pipe (?)
  ...
  m_rgn <- m_rgn %>% # need to double check that this is doing what I want it to be doing 
...
########################## Troubleshooting 17-Jul (delete later)
comp_new <- h_usd %>%
  filter(year == 2015) %>%
#  filter(rgn_name == "United States") %>% 
  filter(product == "fish_oil") %>% 
  left_join(h_tonnes, by = c("rgn_id", "rgn_name", "commodity", "product", "year"))

h_tonnes_old <- read.csv(here('globalprep/np/v2018/int/tonnes.csv'))

h_usd_old <- read.csv(here('globalprep/np/v2018/int/usd.csv'))

comp_old <- h_usd_old %>% 
  filter(year == 2015) %>%
#  filter(rgn_name == "United States") %>% 
  filter(product == "fish_oil") %>% 
  left_join(h_tonnes_old, by = c("rgn_id", "rgn_name", "commodity", "product", "year"))

ggplotly(ggplot(comp_old, aes(x = tonnes, y = usd, labels = rgn_name)) +
  geom_point() +
  geom_abline(slope = 1, intercept = 0, color = "red"))

ggplotly(ggplot(comp_new, aes(x = tonnes, y = usd, labels = rgn_name)) +
  geom_point() +
  geom_abline(slope = 1, intercept = 0, color = "red"))


mismatch_new_tonnes <- h_usd %>%
#  filter(year == 2015) %>% 
  left_join(h_tonnes, by = c("rgn_id", "rgn_name", "commodity", "product", "year")) %>% 
  filter(tonnes == 0.0 & usd > 0)

mismatch_new <- h_usd %>% 
#  filter(year == 2015) %>% 
  left_join(h_tonnes, by = c("rgn_id", "rgn_name", "commodity", "product", "year")) %>% 
  filter(usd == 0.0 & tonnes > 0) %>% 
  bind_rows(mismatch_new_tonnes)

mismatch_old_tonnes <- h_usd_old %>%
#  filter(year == 2015) %>% 
  left_join(h_tonnes_old, by = c("rgn_id", "rgn_name", "commodity", "product", "year")) %>% 
  filter(tonnes == 0.0 & usd > 0)

mismatch_old <- h_usd_old %>% 
#  filter(year == 2015) %>% 
  left_join(h_tonnes_old, by = c("rgn_id", "rgn_name", "commodity", "product", "year")) %>% 
  filter(usd == 0.0 & tonnes > 0) %>% 
  bind_rows(mismatch_old_tonnes)

commodities_old <- h_usd_old %>%
    full_join(h_tonnes_old, by=c('rgn_name', 'rgn_id', 'commodity', 'product', 'year')) %>%
    mutate(commodity = as.character(commodity)) %>%
    arrange(rgn_id, product, commodity, year)

commodities_comp <- commodities_old %>% 
  left_join(commodities, by=c('rgn_name', 'rgn_id', 'commodity', 'product', 'year')) %>% 
  filter(rgn_id == 186) %>% 
  rename(usd_2018 = usd.x) %>% 
  rename(tonnes_2018 = tonnes.x) %>% 
  rename(usd_2019 = usd.y) %>%
  rename(tonnes_2019 = tonnes.y)
...
############ Should we change the data frame names at each step, or is it fine to keep calling it the same thing? 
...
## 'gapfill' will be in (zerofill, endfill, tbd, none) - how are these determined? 
...
# Gap-filling 
commodities <- commodities %>% add_georegion_id() %>% 
...
commod_1 <- commodities %>% np_regr_fill(years_back = 10, vars = 'td', scope = 'rgn_id') # fill in NAs with tons~USD model made from data within the region; this only fills in ~540 lines of data 
regr1 <- commod_1 %>% 
  filter(gapfill == "regr1_u_rgn" | gapfill == "regr1_t_rgn")

commod_2 <- commod_1 %>% 
  filter(gapfill == "tbd" | gapfill == "none" | gapfill == "zerofill" | gapfill == "endfill") %>% 
  np_regr_fill(vars = 'tdy', scope = 'georgn_id') # if region-level data is insufficient, gapfill using georegion level (fills in 10k lines of data)
regr2 <- commod_2 %>% 
  filter(gapfill == "regr2_u_gr" | gapfill == "regr2_t_gr")

commod_3 <- commod_2 %>%
  filter(gapfill == "tbd" | gapfill == "none" | gapfill == "zerofill" | gapfill == "endfill") %>% 
  np_regr_fill(vars = 'tdy', scope = 'global') # fill in remaining NAs with global-level data 

all_gapfilled <- commod_3 %>% 
  rbind(regr1, regr2)
# should match dim of commodities df

### Previous format for performing gapfilling at successive levels (gap-filled using gapfilled data!):
#h <- h %>% np_regr_fill(years_back = 10, vars = 'td', scope = 'rgn_id') # fill in NAs with tons~USD model made from data within the region
#h <- h %>% np_regr_fill(vars = 'tdy', scope = 'georgn_id') # if region-level data is insufficient, gapfill using georegion level
#h <- h %>% np_regr_fill(vars = 'tdy', scope = 'global') # fill in remaining NAs with global-level data 
...
all_gapfilled <- all_gapfilled %>% np_end_fill()
...
# do we need to ungroup here?? Added it above
...
Comparing against last year's data. For example, compare the data for 1976-2015 for the 2018 and 2019 assessment year. Do not need to look at new years of data (2016), which is only available in the 2019 assessment year, since those did not exist in the previous year's assessment.
...
old <- read.csv(here('globalprep/np/v2018/int/tonnes.csv')) %>%  # CHANGE YEAR
  filter(rgn_id==68, year %in% c(1990:2015)) %>% 
...
# v2019: these data zeroes instead of NAs when maybe they shouldn't? new values are >0 

## Look at a few regions that had large differences in status values: Vanuatu, Tunisia, Cuba, North Korea
############# how do we know which regions had large diff in status values? 
...
new_tonnes <- read.csv(here("globalprep/np/v2019/output/np_harvest_tonnes.csv")) %>% 
  filter(rgn_id==6, year %in% c(2010:2015))
...
new <- read.csv(here("globalprep/np/v2019/output/np_harvest_tonnes.csv")) %>% 
  filter(rgn_id==61, year %in% c(2010:2015))
...
##### lots of discrepancies here

## Tunisia harvest smoothed data to see rel tonnes
####### is this what you should do when you encounter discrepancies in the raw values? 
...
new <- read.csv(here("globalprep/np/v2019/int/tonnes.csv"))
old <- read.csv(here("globalprep/np/v2016/int/tonnes.csv")) # CHANGE YEAR
new_c <- new %>% 
...
new_tonnes <- read.csv(here("globalprep/np/v2019/output/np_harvest_tonnes.csv")) %>% 
  filter(rgn_id==112, year %in% c(2007:2015))
old_tonnes <- read.csv(here("globalprep/np/v2018/output/np_harvest_tonnes.csv")) %>% 
  filter(rgn_id==112, year %in% c(2007:2015)) %>% 
  rename(tonnes_old = tonnes)
...
smooth_new <- read.csv(here("globalprep/np/v2019/int/np_harvest_smoothed_data.csv")) %>% 
  filter(rgn_id==112, year %in% c(2010:2015)) %>% 
  select(rgn_id, product, year, tonnes, tonnes_rel)
...
smooth_new <- read.csv(here("globalprep/np/v2019/int/np_harvest_smoothed_data.csv")) %>% 
  filter(rgn_id==21, year %in% c(2010:2015)) %>% 
  select(rgn_id, product, year, tonnes, tonnes_rel)
smooth_old <- read.csv(here("globalprep/np/v2018/int/np_harvest_smoothed_data.csv")) %>% 
  filter(rgn_id==21, year %in% c(2010:2015)) %>% 
  select(rgn_id, product, year, tonnes_old = tonnes, tonnes_rel_old = tonnes_rel)
...
plot(compare$tonnes_rel_old, compare$tonnes_rel)
abline(0,1, col="red")
...
new <- read.csv('globalprep/np/v2019/int/usd.csv')
old <- read.csv('globalprep/np/v2016/int/usd.csv') # CHANGE YEAR
...
new <- read.csv('globalprep/np/v2019/output/np_harvest_tonnes.csv')
old <- read.csv('globalprep/np/v2018/output/np_harvest_tonnes.csv')
