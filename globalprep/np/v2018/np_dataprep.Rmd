...
compare <- old %>% 
left_join(new, by = c("commodity","year","rgn_id","rgn_name","product"))
...
compare <- old_c %>% 
left_join(new_c, by = c("commodity","year","rgn_id","rgn_name","product")); View(compare)
...
compare <- old_tonnes %>% 
full_join(new_tonnes, by = c("rgn_id","product","year")); View(compare)
...
compare <- old %>% 
full_join(new, by = c("rgn_id","product","year")); View(compare)
...
smoothed <- smooth_old %>% 
  full_join(smooth_new, by = c("rgn_id","product","year")); View(smoothed)




## Take a look at Cuba: no diff in intermediate tonnes file
new_c <- filter(new, rgn_id==112) %>%
  arrange(commodity, year) %>% 
  rename(tonnes_new = "tonnes")
old_c <- filter(old, rgn_id==112) %>%
  arrange(commodity, year)
...
## Take a look at Cuba output for usd weights
new_w <- read.csv("output/np_harvest_weights_from_usd.csv") %>% 
  filter(rgn_id==112) %>% 
  select(rgn_id, product,weight)
old_w <- read.csv("../v2016/output/np_harvest_weights_from_usd.csv") %>% 
  filter(rgn_id==112) %>% 
  rename(weight_old = weight) %>% 
  select(rgn_id, product,weight_old)

compare <- old_w %>% 
left_join(new_w, by = c("rgn_id","product")); View(compare)
...
compare <- old_tonnes %>% 
left_join(new_tonnes, by = c("rgn_id","product","year")); View(compare)
