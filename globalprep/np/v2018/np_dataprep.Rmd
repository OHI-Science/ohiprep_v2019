...
## Take a look at Tunisia commodities tonnes: differences start in raw FAO data
new <- read.csv("int/tonnes.csv")
old <- read.csv("../v2016/int/tonnes.csv") # CHANGE YEAR
new_c <- filter(new, rgn_id==61) %>%
  arrange(commodity, year) %>% 
  rename(tonnes_new = "tonnes")
old_c <- filter(old, rgn_id==61) %>%
  arrange(commodity, year)
