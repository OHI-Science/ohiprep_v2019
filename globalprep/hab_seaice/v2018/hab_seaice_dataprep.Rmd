...
An additional year of data has been added. 

**We attempted to transition to using the sf package for the spatial data, however, this was mostly not possible due to our reliance on the raster::extract function which is not supported for the sf format.  Maybe next year we can make this update.**
...
final.year <- 2016 # final year of data ***2 months of 2017 data...***
...
i.pym = 0
t0 = Sys.time() # not really helpful when using rmd...
```
...
### Function 2
Using the data from the .rdata files created in Function 1, this function calculates status and trend for shoreline ice and ice edge habitat. Data is saved in intermediate (`int`) folder: 
...
ref.years <- 1979:2000 # reference years are chosen... how/why?
source("Status_Trend.R") # calculates status and trend for shoreline ice and ice edge hab
```
...
old <- read.csv("../v2016/output/hab_ice_health_eez_updated.csv") %>% # compare to last year's data
...
## extent
extent <- read.csv("output/hab_ice_extent_eez.csv") # compare to last year's data
