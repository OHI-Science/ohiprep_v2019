...
New data was not available for 2017.
...
final.year <- 2016 # final year of data
...
extent <- data %>%
   dplyr::filter(rgn_typ == "eez") %>%
   dplyr::mutate(year = final.year) %>% # extent for most recent/last year of data, final.year defined above
...
old <- read.csv(sprintf("../%s/output/hab_ice_health_eez.csv", previous_yr)) %>% # compare to last year's data
...
old <- read.csv(sprintf("../%s/output/hab_ice_trend_eez.csv", previous_yr)) %>%
...
old <- read.csv(sprintf("../%s/output/hab_ice_extent_eez.csv", previous_yr)) %>%
