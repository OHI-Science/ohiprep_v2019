...
## fyi takes forever, about 6hrs for v2018...
...
# 
# rgn_stats_long <- foreach(i = 1:nlayers(stack_slr), .packages = c("raster", "dplyr"), .combine = "rbind") %do% {
#   yr <- as.integer(str_sub(rasts[i], -8, -5))
#   slr_rast <- stack_slr[[i]]
# 
#   zonal_cmp_fun <- compiler::cmpfun(function(x){
#     rgn_mean <- cellStats(rasterize(x, crop(slr_rast, as.matrix(extent(x))), mask = TRUE), 
#                           stat = "mean", na.rm = TRUE)
#   })
# 
#   foreach(j = rgn_data$rgn_id, .packages = c("dplyr"), .combine = "rbind") %dopar% {
#     rgn <- zones_shp %>% filter(rgn_id == j & rgn_type == "eez")
#     vec[1] <- yr
#     vec[2] <- j
#     vec[3] <- zonal_cmp_fun(rgn)
#     rgn_stats_long <- vec
#   }
# }
# 
