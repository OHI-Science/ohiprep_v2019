######################################################################################################################
## Timekeeping for data download -- setup
######################################################################################################################

t0 = Sys.time()

years = c(1993:(final.year-1))
months = str_pad(1:12, 2, pad = "0")

n.pym = length(years)*length(months)
i.pym = 0
...
userpwd <- userpwd="eleanorecampbell%40ucsb.edu:HZnUkN"


xMin = 0; yMin = -90; n_row = 720; n_col = 1440
xMax = xMin + (pixel*n_col); yMax = yMin + (pixel*n_row)
...
######################################################################################################################
## Collect the data for each month/year and add to raster stack
######################################################################################################################

destination <- file.path(dir_anx_aviso, sprintf("d%s", scen_year))
...
for (yr in years){
  for (mo in months){ 
    ...
    ## yr="1993"; mo="01" # testing
    
    ## retrieve data using ftp
    ...
    print(sprintf("Retrieving %s of %s. Minutes done=%0.1f, to go=%0.1f",
                   i.pym, n.pym, min.done, min.togo)) # time remaining for data download
    ...
    u <- sprintf("%s/dt_global_allsat_msla_h_y%s_m%s.nc.gz", ub, yr, mo)
    con <- file(u, "rb")  # "rb" = "open for reading in binary mode"
    x <- readBin(con, "raw", 300)
    x <- readBin(con,"numeric", signed = TRUE, 1036800)
    close(con)  
