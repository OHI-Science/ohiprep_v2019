...
catch_npp_fun <- function(file, layer){ # file = dem_d_files[1]  layer = 'dem_dest'
  ...
  catch_resmp <- catch %>%
        projectRaster(npp, method = 'ngb', over=TRUE, filename = file.path(dir_M,"git-annex/globalprep/prs_fish/v2018/tmp/temp_resample.tif"), overwrite=TRUE)
...
Create corrected catch rasters. Resolution and projection are matched with NPP rasters (8 x 10 km resolution and Mollweide coordinate system).
...
plot(raster(art_disc[30]))
zoom(raster(art_disc[30]))
click(raster(art_disc[30]))
...
ref_clb <- quantile(vals, prob = 0.9999, na.rm=T)  #11.88
...
ref_chb <- quantile(vals, prob = 0.9999, na.rm=T)  #9.87
...
foreach (i = 2003:2011) %dopar%{ # i = 2003
  ...
  out_CL <- mean_CL[which(str_detect(mean_CL, pattern = paste0("_",i,"_")))] %>%
            ...
            resample(ocean, method = 'ngb', filename = paste0(dir_M, '/git-annex/globalprep/prs_fish/v2018/output/comm_low_bycatch/lb_fish_pressure_', yrs[1], '_', yrs[5], '.tif'), overwrite = TRUE)
# 
#   out_CD <- mean_CD[which(str_detect(mean_CD, pattern = paste0("_",i,"_")))] %>%
#             raster() %>%
#             calc(fun=function(x){ifelse(x > ref_chb, 1, x/ref_chb)}) %>%
#             calc(fun=function(x){ifelse(x < 0, 0, x)}) %>%
#             resample(ocean, method = 'ngb', filename = paste0(dir_M, '/git-annex/globalprep/prs_fish/v2018/output/comm_high_bycatch/hb_fish_pressure_',yrs[1],'_',yrs[5], '.tif'), overwrite=TRUE)
# 
# 
...
#             calc(fun = function(x){ifelse(x < 0, 0, x)}) %>%
#             resample(ocean, method = 'ngb', filename = paste0(dir_M, '/git-annex/globalprep/prs_fish/v2018/output/art_low_bycatch/art_fish_pressure_', yrs[1], '_', yrs[5], '.tif'), overwrite=TRUE)
...
outFolder <- file.path(dir_M,"git-annex/globalprep/prs_fish/v2018/output")
...
al <- raster(file.path(outFolder, 'art_low_bycatch/art_fish_pressure_2011_2015.tif'))
