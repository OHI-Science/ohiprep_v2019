...
Set up foreach loop to start with the oldest year of data (2003) and end on the 5th most recent year of data (2011).
...
foreach (i = 2003:2011) %dopar%{ # i = 2010
  ...
  out_CL <- commL_corr[which(str_detect(commL_corr, pattern = paste(yrs,collapse = '|')))] %>%
            ...
            calc(fun=function(x){log(x+1)}, filename = paste0('int/comm_landings/mean_catch_',yrs[1],'_',yrs[5],'.tif'),overwrite=TRUE)
  
  out_CD <- commD_corr[which(str_detect(commD_corr, pattern = paste(yrs,collapse = '|')))] %>%
            stack() %>%
            calc(fun=function(x){mean(x,na.rm=TRUE)}) %>%
            calc(fun=function(x){log(x+1)}, filename = paste0('int/comm_discards/mean_catch_',yrs[1],'_',yrs[5],'.tif'), overwrite=TRUE)
  
  out_AL <- artL_corr[which(str_detect(artL_corr, pattern = paste(yrs,collapse = '|')))] %>%
            stack()%>%
            calc(fun=function(x){mean(x,na.rm=T)})%>%
            calc(fun=function(x){log(x+1)}, filename = paste0('int/artisanal_landings/mean_catch_',yrs[1],'_',yrs[5],'.tif'),overwrite=T)
  
  out_AD <- artD_corr[which(str_detect(artD_corr, pattern = paste(yrs,collapse = '|')))] %>%
            stack()%>%
            calc(fun=function(x){mean(x,na.rm=T)})%>%
            calc(fun=function(x){log(x+1)}, filename = paste0('int/artisanal_discards/mean_catch_',yrs[1],'_',yrs[5],'.tif'),overwrite=T)
