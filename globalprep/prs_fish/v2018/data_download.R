...
## Download from web and save into mazu
for(web_year in web_years){ #web_year <- "_1950_1954"

data <- read.csv(sprintf("http://data.imas.utas.edu.au/attachments/ff1274e1-c0ab-411b-a8a2-5a12eb27f2c0/Catch%s.csv", web_year))

saveRDS(data, 
        file.path(dir_M, sprintf("git-annex/globalprep/_raw_data/IMAS_GlobalFisheriesLandings/Catch%s.rds", web_year)))
}
