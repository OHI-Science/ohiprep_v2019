...
master <- indexInd %>%
  ...
  left_join(taxa, by = c("Taxonkey" = "TaxonKey")) %>%
    select(ID, Year = IYear, CountryName = FAO.name, TaxonName, CommonName, IndReported = Reported.x, IndIUU = IUUTotal.x, IndDiscards = Discards.x, NIndReported = Reported.y, NIndIUU = IUUTotal.y, NIndDiscards = Discards.y )
