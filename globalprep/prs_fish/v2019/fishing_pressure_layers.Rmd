...
This script prepares and formats the IMAS Global Fisheries Catch raw data into intermediate data by combining Industrial (CatchInd_XXXX_XXXX) and Non-Industrial (CatchNInd_XXXX_XXXX) rds files with IndexInd.csv, IndexNInd.csv, and Cells.csv as well as a single file with all years.
...
* Previously, the raw catch data was all in a single file. This year, we have to combine across five different data tables: catch data (CatchInd_XXXX_XXXX), two master index files split by non industrial and industrial with country name, species, etc (IndexInd/IndexNInd.csv), geospatial information (Cells.csv), taxon information (taxa.csv), and country information (country.csv).
...
rawFolder <- file.path(dir_M, "git-annex/globalprep/_raw_data/IMAS_GlobalFisheriesLandingsv4/d2019")
...
* Spatial Cells Reference (Cells.csv) - contains geospatial information associated wtih the Industrial Catch data
* Gear Reference (Gear.csv) - contains information regarding how different fishing gear is classified in the index datasets. This will probably not be used for the fishing pressure layers. 
* Taxa Reference (Taxa.csv) - contains information regarding how different taxa is classified in the index datasets.
* Country Reference (Country.csv) - contains informations regarding how different countries are labeled in the index datasets. 
...
ind_files <- dir(file.path(dir_M,"git-annex/globalprep/_raw_data/IMAS_GlobalFisheriesLandingsv4/d2019"), pattern ="CatchInd", full.names=TRUE)
...
nind_files <- dir(file.path(dir_M,"git-annex/globalprep/_raw_data/IMAS_GlobalFisheriesLandingsv4/d2019"), pattern ="CatchNInd", full.names=TRUE)
...
foreach(yr = years) %dopar% { 
 ...
 #raw_data <- readRDS(file.path("/home/shares/ohi/git-annex/globalprep/prs_fish/v2019/int/annual_catch/CatchInd_2003.rds"))
...
#rast_test <- raster::subs(saup_rast, landings, by = "Cell", which = "cell_catch", subsWithNA=TRUE)
...
#Check compared to last years... looks very different 
#raw_data1 <- readRDS(file.path("/home/shares/ohi/git-annex/globalprep/prs_fish/v2018/int/annual_catch/CatchNInd_2003.rds"))
# landings1 <- raw_data1 %>%
...
#raw_data1 <- readRDS(file.path("/home/shares/ohi/git-annex/globalprep/prs_fish/v2018/int/annual_catch/CatchInd_2003.rds"))
# landings1 <- raw_data1 %>%
#    dplyr::mutate(Landings_CR = (Landings * 1000)/Area) %>% # convert to catch rate (kg/km^2)
#    dplyr::group_by(Cell) %>%
#    dplyr::summarise(cell_catch = sum(Landings_CR, na.rm=TRUE)) %>% # usually no NAs, but just in case
...
#rast_test <- raster::subs(saup_rast, landings1, by = "Cell", which = "cell_catch", subsWithNA=TRUE)
```
...
#commercial_ly <- list.files(paste0(file.path("/home/shares/ohi/git-annex/globalprep/prs_fish/v2018/int/"), "comm_landings"), full.names = TRUE)
#commercial_ly <- commercial_ly[!(str_detect(commercial_ly, "mean_catch"))] # remove files with 'mean_catch' in file path
#commercial_ly <- commercial_ly[!(str_detect(commercial_ly, "corr"))]
#catch_ly <- raster(commercial_ly[17])
#plot(catch_ly)
#res(catch_ly) # 0.5 degree cells
...
#log_Catch_ly <- raster::calc(tot_Catch_ly, function(x){log(x+1)})
...
#plot(log_Catch_ly, col=cols, alpha=1)
#click(log_Catch_ly)
#zoom(log_Catch_ly)
