...
2. regions_shape()
3. fasterize function from fasterize package, you will use the regions raster as the base raster. I think this will catch points and polygons (but check this). 
4. Make sure all raster values are NA or 0 and 1
5. zonal in raster package with sum function

probably assume each point is a km^2
do fasterize with points and polygons
sum those together somehow 
- stack the rasters 
- calc function, na.rm = TRUE
- some spots equal 2 => anything 1 or greater = 1
- then zonal with zones and newly created combined seagrass raster - function is sum and na.rm = TRUE
 -- sum up all seagrass cells for each zone 
- to get km^2 -- 0.9344789**2*zonal_sums 
...
## Need to assign all "2's" as "1's" and all "0's" as "NA"
...
**Calculate zonal stats with zones raster and new combined seagrass. Convert to km^2**
```{r}
zonal_sums_combined <- raster::zonal(combined_seagrass, zones, fun = "sum", na.rm = TRUE)
...
write_csv(zonal_sums_km2, file.path(dir_git, 'data/habitat_extent_seagrass.csv'))
...
compare_hab <- compare_habitat_extent %>%
