...
Gridded world population densities from CIESE and CIAT needs to be projected into the World Mollweide coordinate reference system and resampled using nearest neighbor method to match the ocean raster (934.4789 x 934.4789 meter resolution); missing years must be filled by interpolation

```{r manipulate spatial data}
## year to numeric value, project to correct crs, resample, and overwrite the data with these updates
for(rast in raw){
  yr <- as.numeric(as.character(str_sub(rast, -8, -5))) # eigth-to-last to fifth-to-last characters of filename give year
  raster(rast) %>%
