...
Gridded world population densities from CIESE and CIAT are projected into the World Mollweide coordinate reference system and resampled using nearest neighbor method to match the ocean raster (934.4789 x 934.4789 meter resolution); data for missing years must be generated by interpolation.
...
eez_25mi_inland <- raster::merge(eez_raster, inland_raster, filename = file.path(path_mar_prs, "int/eez_25mi_inland.tif"))
```
...
## changed to eez_25mi_inland from zones bc zones is defined in spatial_commons.R to be the regions_eez_with_fao_ant.tif
eez_25mi_inland <- raster(file.path(path_mar_prs, "int/eez_25mi_inland.tif"))
pop_stack <- raster::stack(list.files(file.path(path_mar_prs, "int"), pattern = "count_\\d{4}_mol.tif$", full=TRUE))
coastal_pop <- raster::zonal(pop_stack, eez_25mi_inland, fun="sum", progress="text")

write.csv(coastal_pop, paste0(path_mar_prs, "/int"), row.names=FALSE)
...
```{r calculate area and mar. pressure}
...
write.csv(area, sprintf("path_mar_prs/%s/int/area_km2_25mi.csv", scenario_yr), row.names=FALSE) # or save to github repo??
...
write.csv(pressure_data, "globalprep/mar_prs_population/v2017/output/prs_pop_density.csv", row.names = FALSE)
```
...
```{r check areas against previous year's}
