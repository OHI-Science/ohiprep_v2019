...
foreach(i = raw, .packages = c("raster", "rgdal", "stringr")) %dopar% {
  ...
  raster::raster(i) %>%
    raster::projectRaster(crs = crs(ocean), res = res(ocean), over = TRUE, method = "ngb",
                          ...
                          overwrite = TRUE) # ... actually taking TOO LONG, NOT FINISHED AN HOUR LATER...
}
stopCluster(cl)
closeAllConnections()
...
if (!file.exists(file.path(dir_M, "git-annex/globalprep/spatial/v2017/EEZ_inland_50mi"))){
  sf::st_write(inland, dsn = file.path(dir_M, "git-annex/globalprep/spatial/v2017/EEZ_inland_50mi"), 
