...
Density data are converted to population by multiplying density times the area of the cell (934.4789 x 934.4789m) and converting to square km from square meters, to get people per square km. GWPv4 provides both densities and population counts, but, we derive population counts from density because **(?)**
...
# cl <- makeCluster(length(den_raster))
# registerDoParallel(cl)
# 
...
## stack rasters of population counts for all years, sum these, and save this as  a csv called coastal_pop
...
write.csv(coastal_pop2, "output/mar_pop_25mi.csv", row.names = FALSE)
```
...
prev_scen_yr <- paste0("v", as.numeric(substr(scenario_yr, 2, 4))-1)
