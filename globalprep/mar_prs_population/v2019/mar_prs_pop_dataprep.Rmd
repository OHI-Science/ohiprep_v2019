...
# Updating this source file, but it isn't being found; should I move this from the 2018 folder or are we just using the functions in common.R now? 
source('http://ohi-science.org/ohiprep_v2019/workflow/R/spatial_common.R')
...
# not finding this bc spatial_common is not in the workflow/R folder, but there is a zones function in line 62 of common.R? Is there a way to reference this without opening up the file and running this line?
...
foreach(i = raw, .packages = c("raster", "rgdal", "stringr")) %dopar% {
  ...
  raster::raster(i) %>%
    raster::projectRaster(crs = crs(ocean), res = res(ocean), over = TRUE, method = "ngb",
                          ...
                          overwrite = TRUE)
} # using "ocean" found on line 69 of common.R
# this liine is taking forever! 
stopCluster(cl)
closeAllConnections()
...
files <- list.files(file.path(dir_github, "int"), pattern = "density_\\d{4}_mol.tif$", full = TRUE)
...
yearly_diff <- function(year_min, year_max, density_files = files){
  rast_diff <- stack(density_files[grep(year_max, density_files)], density_files[grep(year_min, density_files)]) %>%
    overlay(fun = function(x, y){(x - y)/5},
            filename = file.path(dir_github, sprintf("int/yearly_change_%s_to_%s.tif", year_min, year_max)),
            overwrite = TRUE)
}
...
yearly_interpolate <- function(year, start_raster, yearly_change){
    for(i in 1:4){
      raster::overlay(raster::raster(start_raster), raster::raster(yearly_change), fun = function(x, y){(x + i*y)},
              filename = file.path(dir_github, sprintf("int/human_density_%s_mol.tif", (year+i))),
              overwrite = TRUE)
    }
}

## interpolate missing years
files <- list.files(file.path(dir_github, "int"), pattern = ".tif$", full = TRUE)

# same error message as 201-205 lines above
...
den_raster <- list.files(file.path(dir_github, "int"), pattern = "density_\\d{4}_mol.tif$", full = TRUE)
...
foreach(i = den_raster, .packages = c("raster", "rgdal", "stringr")) %dopar% {
  ...
  raster::calc(tmp, fun = function(x){x * (934.4789 * 934.4789 * 0.000001)},
       filename = file.path(dir_github, sprintf("int/human_count_%s_mol.tif", yr)),
       overwrite=TRUE)
}
stopCluster(cl)
closeAllConnections()
...
#        filename = file.path(dir_github, sprintf("int/human_count_%s_mol.tif", yr)),
#        overwrite = TRUE)
#   print(file.path(dir_github, sprintf("int/human_count_%s_mol.tif", yr))) # check
# }
```
...
# not able to use select here for some reason
...
eez_25mi_inland <- raster::merge(eez_raster, inland_raster, 
                                 filename = file.path(dir_github, "int/eez_25mi_inland.tif"))

(These spatial files were created last year and do not need to be recreated becasue there were no changes to the spatial files).
```{r create eez + 25-mile inland raster, eval=FALSE}
# ## re-add Fiji which appears to get cut: include all of Fiji land falling within 50 mile boundary
# fiji <- regions %>%
#   dplyr::filter(rgn_type == "land" & rgn_id == 18) %>%
...
# raster
eez_25mi_inland <- raster(file.path(dir_github, "int/eez_25mi_inland.tif"))
...
pop_stack <- raster::stack(list.files(file.path(dir_github, "int"), pattern = "count_\\d{4}_mol.tif$", full = TRUE)) # stack
...
pop_rast <- raster(file.path(dir_github, sprintf("int/human_count_%s_mol.tif", compare_yr)))
