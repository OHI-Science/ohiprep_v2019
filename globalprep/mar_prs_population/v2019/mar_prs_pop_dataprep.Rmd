...
v2019: Hosting all spatial files on Mazu server instead of github
...
foreach(i = raw, .packages = c("raster", "rgdal", "stringr")) %dopar% {
  ...
  raster::raster(i) %>% # read in file path to raster
    raster::projectRaster(crs = crs(ocean), res = res(ocean), over = TRUE, method = "ngb", # convert large grids to small grids using nearest neighbor method
                          filename = file.path(dir_M, sprintf("git-annex/globalprep/mar_prs_population/%s/int/human_density_%s_mol.tif", scenario_yr, yr)), # write to the server because the files are so large
                          overwrite = TRUE, progress = "text")
# raster_check <- raster(file.path(dir_M, sprintf("git-annex/globalprep/mar_prs_population/%s/int/human_density_%s_mol.tif", scenario_yr, yr)))
} 
...
# check a random raster from the mazu folder
...
eez_25mi_inland_resample <- raster::resample(eez_25mi_inland, pop_stack, method = "ngb", progress="text") # use nearest neighbor approach to find closest cells rather than estimate between them 

coastal_pop <- raster::zonal(pop_stack, eez_25mi_inland_resample, fun="sum", progress="text") # sum within zones (regions as specified by eez_25mi_inland)
...
### WILL DO THIS SECTION LATER... once we have a csv file with inland areas since right now we only have a combined eez+25mi inland raster (only shapefiles are available on Mazu currently)
```{r calculate area and mar. pressure, eval=FALSE}

# Using the csv created from last year, since we haven't updated area data since 2014: 
area <- read_csv("globalprep/mar_prs_population/v2018/int/area_km2_25mi.csv")
...
# v2018 pulled from 2017 data, changed to 2018 data for v2019
prs <- read.csv("globalprep/mar_prs_population/v2018/output/prs_pop_density.csv") %>%
  dplyr::mutate(pressure_score = 0)

write.csv(prs, "globalprep/mar_prs_population/v2017/output/prs_pop_density_gf.csv", row.names = FALSE) # wouldn't this overwrite old data? Do we want to save this in 2019 folder?
...
## Analysis of name_2_rgn messages (v2019)

# Not matching in v2019: Channel Islands (should we assume these population values are included in the USA population for each year?), Caribbean Netherlands (aka Bonaire, Sint Eustatius and Saba, which are reported separately in OHI), Isle of Man and Palestine (OHI doesn't report these regions). For Caribbean Netherlands, how could we split up the UN population data across the three regions it encompasses? 

# All removed data are from landlocked/disputed regions - looks good 

# Duplicated regions (9): "China", "China, Hong Kong SAR" "China, Macao SAR", "Guadeloupe", "Martinique", "Puerto Rico", "United States Virgin Islands", "Guam", "Northern Mariana Islands" - what does this mean and how do we address this error?

# Also - regions 1-5 aren't included, even though there are population values for them in the pop_rescaled df (was this just to generate pressure scores?)
...
# compare_yr <- substr(scenario_yr, 2, 5) # do we want this to be 2019 even though UN data doesn't go past 2015?
...
pop_counts <- zonal(pop_rast, zones, fun = "sum", na.rm = TRUE) # different extent - resample?

zones_resamp <- raster::resample(zones, pop_rast, method = "ngb", progress="text") # use nearest neighbor approach to find closest cells rather than estimate between them 
pop_counts <- raster::zonal(pop_rast, zones_resamp, fun = "sum", na.rm = TRUE, progress="text") 


pop_UN <- pop_rgn %>%
  dplyr::filter(year == compare_yr) %>%
  dplyr::select(rgn_id, pop_UN = population)
# data are only going up to 2015? 
...
## check plot - should we investigate outliers? 
library(plotly)
