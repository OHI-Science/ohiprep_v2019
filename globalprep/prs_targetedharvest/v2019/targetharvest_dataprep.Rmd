...
output: 
  html_document:
    ...
    highlight: haddock
    includes: 
      in_header: '../../../src/templates/ohi_hdr.html' # delete/change this link? 
  pdf_document:
    toc: true
---

[REFERENCE RMD FILE: https://cdn.rawgit.com/OHI-Science/ohiprep/master/globalprep/np/v2017/targetharvest_dataprep.html]
^ this is old, should we change it to the latest data prep file? 
...
groups <-  c('turtle', 'seal', 'whale', 'sea lion', 'dolphin', 'porpoise') # if seals aren't included, should we remove them from this vector? 

# Going through FAO data species and seeing if they're in our master list of species
## do we need to add species that have changed name? 
...
# error message is being printed that says lots of species are missing from each group, but d_missing_l is empty? does this mean that these species don't exist in the sp2grp table?

# Andrew's beaked whale says it is "MISSING in the lookup the following species in target='whale'" - it is present in the fao_clean file but not in the sp2grp table (only 69 spp here)
...
#l_missing_d = 0, does this mean that the species are in the data but missing in lookup?
# both of these are empty; does that mean it's OK to proceed? 
...
# Filter out duplicates - are there others we need to consider? How did we know these were duplicates? Didn't change anything in the df (still 2200 x 5)
dplyr::filter(m_sum_rgn, country %in% c("Guadeloupe", "Martinique"))

# They will be summed: - not sure what this is doing bc the values are still the same as in the prev iteration of this data frame; decreases number of observations and eliminates country name (now rgn_id)
m_sum_rgn <- m_sum_rgn %>%
...
#quant_95= 160761 <- significance of this? 
...
# all zeroes for gapfill column, does that make sense? 
