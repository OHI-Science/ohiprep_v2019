...
```{r ref_period, eval = F}
## in 2018 assessment gapfilled, testing here, can be deleted next year
## visualize anomalies, effect of gapfilling

pal <- colorRampPalette(rev(brewer.pal(9, "RdBu")))(35)
anomalies <- list.files(file.path(int_sp_data, "annual_anomalies_diff"), 
                        pattern = "anomalies", full.names = TRUE)
anomalies_not_gf <- list.files(file.path(int_sp_data, "annual_anomalies_diff_before_gf"), 
                        pattern = "anomalies", full.names = TRUE)

saveGIF({expr = for (i in 1:length(anomalies)){
  r <- overlay(raster(anomalies[[i]]), raster(anomalies_not_gf[[i]]), fun = function(x, y) {x-y})
  plot(r, 
       col = pal, 
       axes = FALSE, box = FALSE, legend = FALSE, 
       main = sprintf("Number UV Anomalies %s", substr(anomalies[[i]], 99, 102)))
  plot(land, add = TRUE)}},
  ani.width = 800,
  ani.height = 400,
  movie.name = file.path(int_sp_data, "num_anom_gf_diff.gif"))
```
...
## get the reference point (resc_num = rescale number)
vals <- c()
...
foreach(i = 2005:(max(yrs) - 4)) %dopar% {
  ...
  mol1km_masked <- resc_files[str_sub(resc_files, -22, -19) == i] %>%
    ...
    mask(ocean, 
         filename = file.path(out_dir, sprintf("uv_%s_%s-2005_2009_mol_1km.tif", i, max_yr)),
         overwrite = TRUE)
}
```
...
```{r compare_previous_year}

uv_data <- read.csv("output/uv.csv")

old <- uv_data %>% 
  filter(year == 2014) %>% # data years lag assessment yrs by 1
  select(-year, new_pressure = pressure_score) %>% 
  left_join(read.csv("../v2017/output/uv_2015.csv"),
             by = c("rgn_id"))

plot_diff <- ggplot(old, aes(new_pressure, pressure_score)) + 
  geom_point() + 
  geom_abline()
ggplotly(plot_diff)
```
